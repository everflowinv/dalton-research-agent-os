...
class ResearchPlanExecutorTests(unittest.TestCase):
    ...
    def test_company_facts_run_registers_budget_v2_authority(self) -> None:
        ...
        self.assertEqual(
            ...
            "connector-profile-template:sec:0.1:budget-v2:template-v4",
        )
        ...
        policy = harness.connectors.connection.execute(
            ...
            "WHERE policy_version_id='connector-rate-policy:sec-public:budget-v2:template-v4:v1'"
        ).fetchone()
        self.assertIsNotNone(policy)
        self.assertEqual(policy["policy_ref"], "connector-rate-policy:sec-public:budget-v2:template-v4")
        self.assertEqual(
        ...
        price = harness.connectors.connection.execute(
            ...
            "'connector-price:sec-public:zero:budget-v2:template-v4:v1'"
        ).fetchone()
        self.assertIsNotNone(price)
        self.assertEqual(
            price["price_rate_ref"],
            "connector-price:sec-public:zero:budget-v2:template-v4",
        )
        self.assertEqual(
