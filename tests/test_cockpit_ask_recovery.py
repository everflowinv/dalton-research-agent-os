...
class T(unittest.TestCase):
 def setUp(self):
  ...
  review={'status':'ready_for_publication','brain_revision':{'sections':[{'body':'审校回答','gaps':['审校缺口']}]},'review_cost_micros':30};review['content_hash']=content_hash(review);data=(canonical_json(review)+'\n').encode();self.art=self.root/'final.json';self.art.write_bytes(data)
  import hashlib
  ...
  self.result={**self.producer,'display_answer':'审校回答','display_gaps':['审校缺口'],'cost_usd':0.1,'language_review':{'status':'ready_for_publication','artifact_ref':'final','artifact_sha256':hashlib.sha256(data).hexdigest()}}
