...
class NumericCandidateShapeTests(unittest.TestCase):
    def test_the_shape_is_closed(self) -> None:
        ...
        for missing in ("metric", "value", "unit", "period", "basis", "scale"):
            body = candidate()
            body.pop(missing)
            with self.assertRaises(NumericCandidateError):
...
class NumericCandidateVerificationTests(unittest.TestCase):
    ...
    def test_typography_around_numbers_does_not_defeat_the_check(self) -> None:
        for text in (
            ...
            out = verify_numeric_candidate(
                candidate(metric="revenue", value="1234.5", scale="million"),
                {QUOTE_ID: text},
            )
    ...
    def test_a_negative_figure_matches_a_typographic_minus(self) -> None:
        out = verify_numeric_candidate(
            candidate(metric="margin change", value="-1.5", unit="percent",
                      currency=None, scale=None),
    ...
    def test_one_invented_figure_does_not_discard_the_good_ones(self) -> None:
        quotes = {QUOTE_ID: "Revenue was $17.7 billion and bookings were $21.3 billion."}
        verified, refused = verify_numeric_candidates(
            [
                candidate(metric="revenue", value="17.7"),
                candidate(metric="new bookings", value="21.3"),
                candidate(metric="headcount", value="799000", unit="count",
                          currency=None, scale=None),
            ],
            quotes,
        )
        self.assertEqual([item["metric"] for item in verified], ["revenue", "new bookings"])
        self.assertEqual(len(refused), 1)
        self.assertEqual(refused[0]["metric"], "headcount")
