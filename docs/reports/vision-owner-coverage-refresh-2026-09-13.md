...
**R24 已安装并正式发布。** 冻结 runtime `7025`、OPS `2790` 的完整测试共运行 7,949 项（0 failures/errors、1 skip），并通过 134 项 OPS 检查、608 文件与 17 份模型配置核验、14 步复制状态演练及 preflight。安装退出码为 0，同一 controller 的 45/45 健康采样持续 669.670 秒；真实 UI、五家公司导出和一次真实 Ask 均通过。R23 的 Ask 路由失败历史继续保留，高级投资能力仍为 pending。
...
最新 owner 优先级已收敛为先交付正式可运行 demo。R24 已完成页面、模型表、五公司导出及一次真实 Ask 的同版验收并正式发布。最新执行状态见 [PROJECT_STATUS](../PROJECT_STATUS.md)，本次证据与边界见 [R24 发布验收](demo-release-r24-2026-09-13.md)。后续集成包含模板导入、合成双工作区工具和 Wiki 路径修复，但这些扩展不在本次 demo wheel。下列高级范围暂停推进，未从剩余清单中删除。
...
R24 已完成 7,949 项完整测试（0 failures/errors，1 skip）、134 项 OPS 检查、608 文件 wheel/source 核验、17 份模型配置核验、14 步复制状态演练、45/45 持续健康及正式发布。真实 HTTPS 页面 3.382 秒通过，五公司模型/产物页面与 HTML/XLSX 导出通过；一次新 Ask 在 62.214 秒完成并返回带引用结果。页面和单次问答可用不等于高级研究质量、全部内容和人工验收已经完成。
