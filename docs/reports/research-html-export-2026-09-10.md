...
The CLI boundary now rejects collisions between either output and the Core database, asset manifest, every referenced asset, or the other output. HTML and manifest bytes are fully built before either write. Each destination is written through an owner-only `0600` temporary file, flushed, and atomically replaced in its destination directory. The final focused suite passed 16 tests.
