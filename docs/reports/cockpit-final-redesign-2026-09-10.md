...
The two existing browser regressions were executed through `playwright-cli run-code` with page-local fixtures. Approval refresh passed all six checks (draft/focus/selection, pending state, disabled action, same-version refresh, new-version separation, and stale-response rejection). Ask recovery passed all four checks (navigation during a call, eventual result, resumed polling, and completed history). Both reported `writesToLive: 0`.

`scripts/check_cockpit_final_interactions.js` passed seven checks at 390px: structured product source, explicit missing reason, no object-string leak, arrow-key tabs, company close focus, no document overflow, and focus restoration for sources/models/reflection/claims/model/document/operations readers. Switching away from and back to the steering tab retained the exact textarea node, value, and focus. Long reader rows now wrap, while tables scroll inside the reader instead of widening the page.
