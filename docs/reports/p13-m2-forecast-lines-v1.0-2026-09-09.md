...
日期：2026-09-09
分支：`wave1c-forecast-lines`（worktree `dalton-wave1c-forecast-lines-worktree`），基线 main `08c66d0`
...
{
  "schema_version": "0.1",
  "id": "forecast-model-version:0001467373:2",
  "model_ref": "forecast-model:company:sec-cik:0001467373",
  "version": 2, "prior_version_ref": "forecast-model-version:0001467373:1",
  ...
  "assumptions": [{
    ...
    "kind": "estimate",
    "because": "the average quarter-on-quarter change over the 4 quarters filed to 2026-05-31 (0.15%), carried forward unchanged",
...
默认假设生成器：环比增速取**相邻**（期末相差 80–100 天）四对的算术平均；费用份额取最近四个季度 expense/revenue 的平均；tax 取 tax/operating_income 的平均（历史营业利润用**同一条链**算出来，不读 filed 的 operating income——用别的定义算出来的比率不会重放）。`because` 每句都点名平均了哪几个季度。资产负债表本轮不做（规格标 required 也不做，见开放问题 4）。
...
## 5. 接线：LaneSpec 那一行需要什么

Wave 0 的 `LaneSpec(operation, core_only, param_fields, build_coordinator, argv_fragment, launcher_factory, driver_key)`，这条 lane 要的是：

- `operation`：`"model_forecast_lane"`（无 discovery 操作；这条 lane 没有队列）。
- `core_only`：`True`（只读 Core，写 Core）。
- `param_fields`：无。可选 `company_ref` 只在手工跑时用。
- `launcher_factory`：`model_forecast_launcher.ModelForecastLauncher(state_dir=…)`。**不需要 model config、不需要 scheduler db**——这个 child 不调模型。
- `build_coordinator`：`MissionModelForecastLaneCoordinator(missions=…, models=ForecastModelAuthority(store), launcher=…, mission=<callable 返回当前 mission 版本>)`。**注意**：它需要一个 `ForecastModelAuthority`，writer 得在自己的 store 上建一个（和 `CoverageMissionAuthority` 同样的构造方式，会自动 `executescript` 自己的 schema）。
- `argv_fragment`：`["--model-forecast-lane"]` 之类的开关，launchagent 拼进去即可；没有额外参数。
- `driver_key`：`"model_forecast"`，`bounded_planner_driver.run_once` 每 tick 调一次 `dispatch_once()`。tick 频率跟规格 lane 一样即可（5 分钟绰绰有余，静默是常态）。

`dispatch_once()` 的返回状态：`launched` / `idle` / `held` / `busy` / `rejected` / `unconfigured` / `unavailable`，都带 `settled`（上一个 child 的结算）。
...
2. **打包**：`forecast_driver_schema.sql` 没有加进 `pyproject.toml` 的 package-data（按分工不碰那一段）。Wave 0 改成 `*_schema.sql` glob 之后自动覆盖；**在那之前 wheel 装出来会缺这个文件**，合并时请确认 glob 已生效。`test_packaging` 现在是白名单式的，不会因此失败。
...
| DXC | published v1 | 6 / 17 | 12 | 12 | 同上（SG&A、D&A、Restructuring） |
