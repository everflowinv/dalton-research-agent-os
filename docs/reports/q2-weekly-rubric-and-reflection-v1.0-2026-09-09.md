...
日期：2026-09-09
状态：分支 `q2-reflection`，基线 main `61f4255`（2,627 项测试），待合并
...
| --- | --- | --- | --- | --- |
| `rubric:weekly-brief` | 1 | `weekly_brief_issue_version` | 10 | `ca2c06585715903e456bf4b14d309f52f46380e52643741d71f0bc003046d740` |
...
**判官仍要为被闸住的标准各返回一条分数**——回复合同要求每条标准恰好一条，而改 `validate_judge_output` 就是改前三份评分表已经绑定的判读契约。但那一分评的是「这份简报有没有诚实说明该能力尚未接入」，记录里它以 `not_applicable_yet` 发布。这是本片最明显的一处妥协，见第 8 节开放问题 2。
...
- **不写 Ledger**：模块只打开自己的两张表。`FreezeTests::test_the_module_imports_nothing_that_could_write_the_ledger` 逐行扫 import；`test_the_only_writes_are_its_own_two_tables` 装一个 `sqlite3` authorizer，把两张表以外的任何 INSERT/UPDATE/DELETE 判 `SQLITE_DENY`，然后跑完整条写入路径；`test_computing_a_reflection_writes_nothing_at_all` 把连接设成拒绝一切写，再算一遍。
...
| 新登记问题 vs 被回答 | `backlog_question_events.state` | 新 1 / 答 0 / 周末未决 67 |
| 退役 Claim | `claim_retirement_decisions` | 退役 0、保留 0、新挑战 0、未裁决 0 |
| planner inquiry 派发 / 总数 | `coverage_mission_research_plans.inquiries_json`；派发 = 本周新开的 `bounded_planner_loop_versions` | 0 / 0，附 P14e 未落地的原因 |
...
| 反馈按词计数 | `analyst_journal_entries` + `weekly_brief_feedback`（同一套五词） | 0 条 |
...
问题账本上，本周新登记 1 条、被回答 0 条，周末仍未关闭的有 67 条。新登记多于被回答，未决问题集合在这一周变大了。
Claim 层退役 0 条、保留 0 条，本周新提出 0 条挑战，周末还有 0 条挑战没有裁决。
规划器本周记录 0 份 research plan、提出 0 条 inquiry，其中 0 条被派发。本周没有为任何 inquiry 开出
BoundedPlannerLoop。P14e（D1 的 ResearchTask）尚未落地，`adhoc_research_enabled` 仍为 False，所以
planner 的问题目前只能经由人或下一轮 research plan 被动消化。
闲置率不可算：Core 里没有 tick 账本——run_once 的摘要只写进 run/heartbeat.json，下一次 tick 就覆盖它；
除非有人归档过，没有东西可数
有 1 个 stage 停在 entered 等人，最老的一个已经等了 0.65 天；本周关掉了 0 个。
质量分不可读：research_quality_score_versions 不在这个 Core 里：Q1 的质量分 authority 还没有部署
这一周没有人给过任何一条反馈。五个词的反馈词表还没有 UI，所以「没有反馈」暂时不能读成「读过而没有意见」。
```
...
**本周 reflection**：窗口 `2026-08-31T00:00:00-04:00 → 2026-09-07T00:00:00-04:00`，`iso_week=2026-W36`，`inputs_hash=20f6860a…`；八项指标里 6 项可读、2 项按原因报缺（闲置率、质量分）；1 条 backlog candidate、3 条 policy suggestion。数字见 4.2 与 4.3。
...
2. **被闸住的标准仍然要判官返回一个数字，这是妥协。** 干净的做法是让判读契约接受 `"score": "not_applicable_yet"`，但那要改 `validate_judge_output`——前三份评分表已经绑定的契约。建议：等第一次真实判读跑完、确认判官确实会照 grading note 去评「有没有诚实说明」之后，再决定要不要把 `not_applicable_yet` 提升成一等的分数值，那时是一次显式的判读契约升版加四份评分表重出哈希。
...
4. **候选生成规则是我写的，没有第二个人看过。** 和 Q1 的第四条开放问题同一个形状，而且更尖锐：这一片的产出是「系统怎么评价自己的一周」。建议 owner 拿 2026-W36 这条 reflection 的散文与候选各读一遍，说哪一句是废话、哪一条候选不该被提出来。
...
7. **`work_order_family` 是一次前缀解析。** 它今天正确（八个前缀全部在代码里拼出，逐一核对过），但它是约定而不是契约。C2 落地时应当作废它。

---

## 9. 验收结果（原文）

全量测试，`PYTHONPATH=$PWD/src .venv/bin/python -m unittest discover -s tests -t .`：

```
Ran 2723 tests in 366.952s

OK (skipped=1)
```

基线 main `61f4255` 是 2,627 项，本片新增 **96 项**。分项：

```
tests.test_research_quality_weekly_brief: Ran 28 tests in 0.027s
tests.test_research_cycle_reflection:     Ran 48 tests in 0.679s
tests.test_mission_reflection_lane:       Ran 19 tests in 0.159s
tests.test_research_quality_rubrics:      Ran 16 tests in 0.002s   （Q1 的 15 项，净 +1）
tests.test_research_quality_golden:       Ran 14 tests in 0.156s   （项数不变，多跑 5 例周报 golden）
```

没有失败、没有静默跳过。`tests/test_service.py` 与 `tests/test_lane_registry.py` 的字面量一个字没改（前者的 launchagent argv 断言、后者的假 lane argv 断言都靠 `argv_fragment` 的 `core.sqlite` 门通过）。

**没做的事**，如实列出：

- 没有跑过任何一次真实模型判读，所以周报的分数区间没有被验证过。
- 没有第三期 live 周报（不存在）。
