...
## Complete selection-before-download candidate — 07:35 UTC

The integrated 0.2 selection context distinguishes quarterly calls from sell-side research. Both use title/snippet/date/type/tag metadata before downloading full text; company tags remain hints. Sell-side selection receives complete current mission questions and may select a peer or industry document only with a concrete research-use reason. External metadata is explicitly data rather than instructions. Tagged metadata projections retain truncation markers/full-list hashes and fit twenty-candidate Unicode pages under the reviewed input budget; the model route and budget remain Cockpit-managed.

A missing selector/configuration on a selected 0.6 plan now holds both specs and blocks searches/acquisitions/retries, rather than silently falling back to the old full-download path. Legacy 0.1 plans and no-context prompt replay retain their contract. Sell-side consumption does not invalidate merely because missing call quarters change. The JSON schema now validates 0.6 company/query variants as well as old plans.

Real child-process → Unix broker fixture → adapter/Cockpit → formal Scheduler → coordinator tests separately cover earnings and sell-side selection, selected-only fetches, consumption and idle. They prove software execution with controlled providers, not relevance quality from an actual paid model invocation. The latest actual IBM metadata rehearsal used twenty hash-bound archived candidates, a 26,166-byte prompt and successful temporary Scheduler reservation/claim, with zero external/model/paid calls. Combined integration passed 188 tests; activation remains pending the next accepted release.
