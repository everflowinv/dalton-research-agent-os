...
- AlphaEngine `get_document`：80 次/日；每一页算一次；
...
- AlphaEngine live `search_library/get_document` 测试 profile 已绑定 50/80 次日限额；
...
本切片没有部署、没有调用 AlphaEngine/Gemini、没有写 live Core，也没有实现完整多页 coordinator 或 Model Input
Ledger。下一切片仍是 bounded multi-page document acquisition。
