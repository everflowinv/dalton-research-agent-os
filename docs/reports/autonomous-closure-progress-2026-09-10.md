...
## 21:57 UTC — full-suite failures and product-path review

The first foundation freeze `925e50c` completed 6,661 tests in 546.467s with 13 failures, 6 errors and 1 skip. Its receipt is failed; the wheel and copied-state rehearsal do not override that result. The next `e9d06c2` run was deliberately interrupted after this result arrived, so its partial run is not acceptance either. Live remains c84d076 and no host repair or redrive has been applied.

- Issuer-proof fixtures had omitted the metadata now required by the production gate. Valid numeric/metric/qualitative fixtures now explicitly register a company earnings-call title and named-company proof; missing/wrong-proof rejection remains enforced. Targeted checks passed, while an isolated public-web child failure is being independently checked rather than ignored.
- The statement horizon fixtures now supply the v0.2 model contract and an actual consolidated revenue row; the unrelated-capability test uses the supported SEC v2 contract. The rehearsal seed inventory now includes the installer's proposed v3 record. These 211 contract/rehearsal/governance tests pass; no approved v2 record was changed or v3 approved.
- A real forecast lifecycle gap was found independently: a changed model spec was excluded before calculating a new forecast identity. The candidate now starts a new spec-bound model with the prior head as a concurrency condition, retaining old model and forecast-line bytes. Its 117 author-side tests passed; independent review and integrated checks are pending.
- Dossier now states parser bounds in its prompt and versions the draft contract. Actual producer/verifier content refusals stop unchanged paid retries; verifier transport unavailability retains bounded recovery. All three incremental commits were reviewed separately before integration.
