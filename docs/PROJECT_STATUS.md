...
Owner 将当前范围收敛为“优先开发没有就跑不起来的内容，然后部署”。R22b 安装因部署工具漏接返回值失败，已健康回滚；修复后 R22c（runtime `5dbe5212`、OPS `2cd9f150`）安装成功，608 个运行文件与 17 份模型配置核验通过，45/45 持续健康采样通过并正式发布。此结果只证明运行基础通过，不能代表 demo 页面已验收。

真实页面检查发现总览每 6 秒重复请求会堆积。最小 demo 候选 R23 `3e4e8682` 已修复前端请求合并、后台重复读取和旧数据降级路径，源码与 OPS 同版冻结。当前 wheel 608 文件逐字一致、3 JS 检查通过，完整测试及复制状态演练进行中。固定真实 Core 数据的只读 UI 预验收已显示五家公司模型卡；人为延迟 7 秒跨多个轮询周期时最大并发仍为 1。真实 HTTPS 部署后验收尚未完成，暂不宣称 demo 交付。
