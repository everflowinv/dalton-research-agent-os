...
Current checkpoint (2026-09-11 21:43 UTC): R15 `d1079bafc565e1bfa7d19d3360504fa845e9b327` is frozen; the full suite and fresh copied-state rehearsal are running. Its wheel matches all 606 runtime files (438 Python, 73 SQL, 92 JSON, 3 HTML), with three JavaScript syntax checks. The company-specific 0.4 income/cash-flow companion and AMZN-style Excel export passed independent review; final combined LibreOffice recalculation verified 110 cells, including quarterly/annual income, EPS and day-weighted shares, cash flow and actual driver ratios, with no formula errors or missing cells (maximum residual 4.31167e-9). Final Financials/Driver render inspection passed. The five current legacy models and 500 stored result cells retain identical canonical reads and recomputation against the installed version. Full balance-sheet, cash-flow-detail, segment/category and Street breadth remain explicitly unfinished. The bounded daily-budget recovery fix and Cockpit plain-language status changes are included; current visual styling is retained. Live remains verified R14c1; its product audit showed two directed admissions/local searches but zero proven paid sends or promotions, so neither the candidate tests nor successful installation will count as research completion. Deployment is now held on a newly found production gap: a schema-valid but numerically false structure can be persisted as current before historical replay, leaving the forecast lane blocked. A bounded pre-persistence replay fix is in progress; the frozen R15 suite continues as historical evidence. Next: validate real financial inputs before spec persistence, independently review the fix, then freeze and accept a successor before deployment. Numeric-period and note evidence in the modelling prompt remains a separate foundation follow-up. Human source signatures remain separate. See [R14c1 runtime and product evidence](reports/foundation-r14c1-runtime-acceptance-2026-09-11.md), [AMZN format acceptance](reports/fund-amzn-template-parity-review-2026-09-11.md), and [cash-flow companion review](reports/structured-cash-flow-companion-review-2026-09-11.md).
