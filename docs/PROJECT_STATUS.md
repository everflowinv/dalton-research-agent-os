...
Owner 将当前范围收敛为“优先开发没有就跑不起来的内容，然后部署”。因此以已通过完整验收的 R22b `5dbe5212` 作为正式 demo 候选，正在执行安装；随后完成持续健康、精确安装核验、正式发布和核心页面/研究产物可用性验收。其余高级投资能力、模板扩展和真实多工作区验收暂停推进，不并入本次冻结。

模板 registry/select 已在独立分支完成 `8e66ee01`、31 项相关测试通过，保留候选但不合入 demo。Conviction v2 与回报归因仅保留设计，未激活。Wiki 路径修复和模板/合成工作区工具在集成分支中，均不属于 R22b wheel；不以集成进度冒充已部署功能。
