...
Current checkpoint (2026-09-11 22:18 UTC): R15a `34ef59cbbdc4210a58be8513c20c1b237022f822` is frozen and pushed; full discovery passed 7,655 tests with zero failures/errors and one skip, and the fresh 14-step/42-tick/zero-escape rehearsal passed. Its 606-file wheel/three embedded JS checks pass. Final actual-company visual QA found clipped unit headers; a header-only correction is in progress before the next freeze and deployment. Both R15 product blockers are fixed and independently reviewed: automatic specs replay real historical financial inputs before becoming current, and legacy 0.1/0.2 inputs retain their exact projection contract while structured/future schemas stay strict. All five real company production Excel exports now succeed; 484 model cells recalculate consistently with no formula errors (max residual 5.8e-11). Combined integration passed 85 tests; prior R15's 17 fixture-related failures and failed receipt remain preserved. Live remains verified R14c1 until exact-source R15a acceptance completes. Next: finish and independently inspect the unit-header correction, freeze and accept that final source, deploy under existing authorization, observe sustained health and separately verify normal research recovery/new-model production. A source-bound numeric-period modelling context is being developed on a separate branch; note authority, complete statement/segment/category/Street breadth and human source signatures remain distinct later work. See [R15/R15a acceptance](reports/foundation-r15-candidate-acceptance-2026-09-11.md) and [AMZN format acceptance](reports/fund-amzn-template-parity-review-2026-09-11.md).
