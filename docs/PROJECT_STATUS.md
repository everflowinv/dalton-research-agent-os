...
Current checkpoint (2026-09-11 13:20 UTC): R11a `424c225` remains deployed_verified (7,164 tests / 1 skip; exact 572-file wheel; 45 healthy samples / 671.149 seconds). Backup retention keeps three snapshots. After configuring ZeroBase producer/verifier single-call limits to $1, actual normal-lane ACN, EPAM and DXC reviews passed independent model verification; exact Core reviews, Scheduler formal outputs/invocations, Work and Router decisions were checked. IBM correctly refused a citation absent from its supplied Work. Successor integration now includes source-neutral Alpha/web/other-filings/sales-note/wiki original-document adapters, non-mutating manifest/spool/receipt readers, verified planner inventory, exact document/version/query selection, and reviewed annual candidate promotion. Original text remains separately readable from Claims. Current-state read audit exposed legacy acquired rows without ticket refs; compatibility is being fixed. Generic execution identity, normal production dispatch, source re-reading and automatic capacity/day-budget recovery are still pending and must pass before successor deployment. Foundations precede advanced investment judgement, final visual/reference deliverables and multi-workspaces.
