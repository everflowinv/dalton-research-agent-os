...
Current checkpoint (2026-09-12 19:45 UTC): R17a remains deployed and runtime-verified (7,730 tests / one skip; 45/45 health samples). R18 integrates truthful downstream dispatch failures, terminal model-lane holds/fairness, exact budget no-send proofs, and an independently reviewed actual-endpoint guard for explicit independent-verifier controls. IBM model-spec is a producer: its existing 6,000-token limit is a post-response acceptance ceiling, so this guard does not prevent IBM's original call. The producer policy and owner model choices remain unchanged. A proved local required-control refusal may use the existing authorized fallback chain with zero local spend; missing or invalid proof still halts with conservative reservation. Integrated fallback tests passed 58 and broker tests 33; the prior combined regression passed 174. Actual model outputs also exposed supplementary basic EPS/shares/dividend-per-share being placed in currency-only income roles. The prompt/schema now explain those boundaries; independent model/spec/state/CLI validation passed 107 tests without weakening semantic whole-refusal. The broker/host deployment contract is still under adversarial review: initial review found historical-manifest compatibility, current frame-size baseline, exact host-target binding, and final installed-proof gaps. R18 is not frozen or deployed. Three GPT-5.6 Sol agents continue deployment implementation, independent review, and inert release preparation. IBM's prior no-send conclusion remains superseded by broker evidence; no uncertain reservation is refunded. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
