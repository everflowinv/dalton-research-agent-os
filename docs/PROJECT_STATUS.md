...
Current checkpoint (2026-09-11 16:59 UTC): R13b `6a2631ff` passed its complete 7,437-test suite (1 skip, 722.02 seconds) and exact 599-file wheel verification, but is NOT deployable: real copied-state rehearsal found that cockpit_setup removes the existing allowed OpenClaw catalog path and install.sh then restores/reformats service.json, violating preservation of the reviewed planner-only budget delta. A minimal runtime preservation/no-op install fix and independent end-to-end setup review are in progress for R13c. Earlier missing scratch document/lane copy was corrected and independently reviewed as ops `46ec4160` (integrated `50a65138`); the second real rehearsal exposed this separate runtime defect. Both failed rehearsal records remain. Live R12a stays installed with failed sustained health; accepted pointers remain R11a. The owner-approved Yahoo source successfully fetched all five companies. The latest read-only baseline has 110 readable originals, four formally verified Dossier v0.3 company records (CTSH absent), and 12 planner cycles stopped before sending by the old input limit. Generic original research has no live admission/outcome yet. The candidate complete prompt fits 242,074 / 250,000 bytes with all source identities and previews retained. Next: finish the actual install-chain fix, freeze/test/build/rehearse R13c, deploy and verify real planner dispatch plus sustained health, then re-audit earlier owner instructions and postponed work. Keep the current Cockpit visual style and useful subtask/run-detail design. On owner’s latest brain-capacity question, retain one coordinating planner with independent executors for now; assess planning latency, queued task age and feedback freshness after fixing the known input refusal, before adding hierarchical industry/company planners. No evidence yet establishes planner compute saturation.
