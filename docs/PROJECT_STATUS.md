...
Current checkpoint (2026-09-11 14:08 UTC): R11a `424c225` remains deployed_verified (7,164 tests / 1 skip; exact 572-file wheel; 45 healthy samples / 671.149 seconds). Backup retention keeps three snapshots. Actual $1 ZeroBase normal-lane ACN/EPAM/DXC reviews are verified. Successor source-neutral directed research now has original registry/search/read, exact planner admission, configurable model selection, four-stage execution, typed insufficient-evidence feedback and planner wake. Actual read-back with Claim-table access prohibited passes for AlphaEngine and SEC; no model calls were made. Complete-text registration checks correct the live inventory to 99 readable originals (89 Alpha, 5 SEC, 5 web), with five web failures explicitly retained. Prior research imports bind original containers and deterministic text. Production UDS integration, real Router capability vocabulary, canonical promotion and bounded capacity/day-budget recovery are still being completed and independently checked; these features are not yet deployed. Foundations precede advanced investment judgement, final visual/reference deliverables and multi-workspaces.
