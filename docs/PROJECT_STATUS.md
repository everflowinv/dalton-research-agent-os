...
Current checkpoint (2026-09-11 17:53 UTC): R13c `4d459e54` remains the verified live release (45/45 sustained health samples). Normal forecast sensitivity rotation has now reached **5/5 companies**, with exact current-fiscal-year four-quarter revenue A/E joins and stored results matching production replay; EPS stays explicitly unavailable. The pending planner frame/accounting repair is merged (`d00d3346`, `3b92e5b6`): configurable shared 1 MiB encoded-frame default, effective execution identity, and zero settlement for definitely-not-sent local request overflow. A reviewed host frame change and successor deployment remain pending. Owner switched OpenClaw web search back to Gemini: actual host selection is Gemini while its broker's legacy expectedProvider is still Antigravity. Dalton-side automatic per-search selection and signed provider guard merged as `c631a74e` (104 focused tests, absolute source path); companion host-plugin hot selection and real protocol tests are in progress. These changes are not yet live. Owner clarified that model rows/formulas must follow each company's filed statements and notes. Existing company-specific spec selection must feed an equally company-specific earnings bridge; the present fixed operating-income-minus-tax formula is an established limitation, not a complete model. Sol is implementing sourced structure/reconciliation foundations before forecast integration. Refreshed owner-gap report and an unsigned prices/calendar/sales-notes/SEC 8-K review packet are prepared; no human signature or source activation has occurred. Next: complete host search + frame transition, freeze/test/rehearse/deploy, prove normal planner→original-research execution, then finish company-specific modeling and remaining foundation gaps. Preserve current Cockpit style and keep one coordinating brain until measured throughput warrants scoped delegation.
