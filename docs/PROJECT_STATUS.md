...
**故障风暴根因链（详见 [reports/work-order-failure-storm-2026-09-15.md](reports/work-order-failure-storm-2026-09-15.md) 与 [reports/crowd-starvation-and-trajectory-fixes-2026-09-15.md](reports/crowd-starvation-and-trajectory-fixes-2026-09-15.md)）**：Claude CLI 订阅周限额的错误文本被分类为不可回退的 contract_violation，导致脑力链整链停摆（`INVALID_HOST_RESULT`/`HOST_COMPLETION_FAILED` 改判 provider_failure）；budget 拒绝同样按环节回退（一次输入超限不再停整链）；宿主帧失败不再按预留上限记当日花费（此前一个被限额挡住的网关一小时空烧几十次预留额，即"花费涨进度不涨"与池耗尽的直接来源）。xreach CLI 入口被包装脚本穿透软链覆盖，自递归 exec 空转，其同步子进程把 writer 请求队列饿死（舆情/投资建议/勾稽三条 lane 同时 RemoteError 的根因）；修复入口并给舆情 lane 加子进程 20 秒上限 + 每轮 10 秒预算。轨迹页三个显示缺陷（字面 HTML、未登记模型、刷新劫持其他页面）同批修复。

**模型路由**：链条耗尽与中途不可路由的返回统一携带逐环节失败明细，cockpit 消息附未试环节的拒绝原因；tier 保存同步 allow-list 至全部可达模型（16 个存量策略已重发布、21 个配置重指向）；antigravity 两个档案的申报输入上限修正为实测传输上限（约 3 万字符，agy CLI 二进制内部缺陷）；验证类用途（16 个）以显式 override 钉定到目录中仅有的两个声明 `provider-controlled-verify` 的模型（broker 侧真实属性，不可谎报），页面在对应行标注"契约钉定"说明与层级选择的关系；目录同步注册的全部模型默认携带全部通用能力（`DEFAULT_CAPABILITIES` 八项，owner 元数据声明优先）。

**plan 提示词**：投影规则升至 0.3，五级递进（预览全裁 → 不可用文档按公司聚合 → 可读文档验证字段瘦身 → 财务模型摘要 → 每公司保留最近 8 份全量其余聚合），实测 298KB → 55KB，且地板不再随抽取队列增长；planner 输入预算 250K → 80K（实测地板 75KB）。

**预算与配额**：研究预算块形状扩为"3 必填 + 2 可选"（mandate 简化的回归曾令全部文档视图被拒、344 份排队一整天）；共享调用预算策略修订至 revision 4 镜像打包默认（revision 2 的 $1,000,000 默认曾把 industry_framework 的验证预留抬到百万、连续 16 轮 unverified）；prior-research 与全部 crowd 源（雪球/X/Blind 七个操作）日上限 50 → 10,000；Guidepoint 配额三处修复（计划文件 20→200、幂等键冲突兼容、超出带外提升的政策采纳）；pools 关闭后不再返回展示数据；总览卡只显示当日实花；"今天"定义为 UTC 自然日（北京时间 08:00 归零）。

**界面与运维**：轨迹页时间改浏览器本地时区；研究环境可在页面重命名（`workspace_rename`，呈现层元数据，含 legacy 环境标题）；新增 `scripts/point_services_at_release.sh` 按模式发现全部 LaunchAgent 切换 release（09-16 上午新环境 ws-7d89 曾因不在硬编码清单里面停留在旧 release、control 启动竞态崩溃，已修复）；测试目录的 availability 窗口改 10 年 TTL（原 7 天窗口在 09-16 08:00Z 整点令 97 个链路测试准时失败）；自动巡检自动化每 45 分钟运行。

## 2026-09-15 当前部署：三类模型、三道预算上限与文风契约 v0.4

源码 `a9f0e90250641812e74fd589befeb31cb387cca7` 已部署至 8793 与 8794（运行包 `8e9101145e539e6fa4594e14e9376df0fa88b7e5c9ab3f07cfdb3406eeb01da3`）。线上已实测整类保存（cheap 类 → 单模型 GLM-5.3 Flash，即时生效）。修复了整类保存在未配置全部运行时固定的主机上的崩溃、同文件多 section 固定互相覆盖的缺陷；针对旧论点版本的从零复盘候选项现在在待办中标注过期并只提供关闭按钮。模型与预算页合并为一页：三类模型（高阶推理 / 批量阅读 / 独立复核）各一条可拖动排序的调用链，整类保存即时生效（含常驻服务的类别提示重启）；预算只保留三道上限——每日总费用、AlphaEngine 次数、每日阅读份数——分项预算池与单次调用费用上限改为只统计（共享单次默认调至 $1,000,000，池执行开关 off，调用次数上限 100,000），全部机制保留可随时恢复。总览页的预算编辑框已移除。

线上任务预算已按上述落地（阅读上限暂设 2000 份/天，可在页面改）。交付物文风契约升级 v0.4：正文不再提及历史版本、按主题分段、防御性否定表达改写为正面陈述。期间修复研究预算授权链在"策略内容真正变化"路径上的存量键名缺陷（create_policy 返回 wire 与契约字典键不同）。阅读进度方面：昨夜预算池耗尽导致的拒单已随池开关解除，ACN 阅读将在下一个抽取轮次自动开始；此前 CTSH/EPAM/IBM 已各有阅读完成记录。

## 2026-09-15 当前部署：时间预算修复、测试全绿与宿主整理

源码 `6c7e9343a6bbbfc203e21a06aa7d6aa477da189d` 已部署至 8793 与 8794，运行包为 `e73d9ee49d58806e7e5ebadddda2b86c74930f6b5d3195fc5bf3674f0da00fdb`。本轮补完 09-14 中断的资料发现时间预算收尾（各维护阶段入口检查截止点、恢复游标只在处理后推进），并把 `profile:deepseek-v4-flash` 的策展路由对齐线上 `deepseek-flash` 别名（V4.1 Flash，$0.15/$0.60），catalog 自动同步的对账漂移归零。

完整测试在安装解释器下执行 **8,525 项，零失败、零错误、4 项跳过**。修复了 09-14 线遗留的三组失败：plan 调用默认提到 $1 后的研究任务池测试算术、发现 tick 预算测试的桩签名、以及 d3c7dca7 预选 reconciliation 之后的两个取件测试断言；r11a 发布演练测试改为运行时源码根缺席时显式跳过。默认任务 $5/天的 25% 池现在装不下一个双轮任务（$2），属策略事实，任务池测试改为显式购买预算。

宿主整理：OpenClaw 模型/搜索 broker 插件迁至 `~/.openclaw/managed-plugins/` 并重启网关验证（重复 id 警告源于双通道注册机制，保留）；450 个 git worktree 清理至 1 个，511 个分支清理至 main，未合并分支以 patch 归档于 `~/Projects/dalton-archive-20260915/`（含 7 份未提交改动）；磁盘可用空间 4.7 GB → 27 GB。

部署后回读：电话会取件维持 4/4，CTSH/EPAM/IBM 的全文阅读证明已从零开始推进（GLM-5.3 Flash），ACN 尚为零；仍不以取件完成等同阅读完成。8794 升级健康检查曾两次因三服务同时冷启动的 SQLite 锁竞争超时回滚（已查明非代码问题），脚本改为耐心轮询后通过；8793 一次因 publication worker label 半注册态 bootstrap I/O 错误回滚，清理 label 后重跑通过。

## 2026-09-14 当前部署：独立研究环境、共享调用预算与资料获取修复

源码 `c422308c008690ce8d084a39ffbea2c2c57f7287` 已部署至 8793 与 8794，运行包为 `b0f9bda78f7db267e879ee0e44cacab13a58ed02f100da26c37eea889fd52f55`。两个环境共用连接基础设施，数据库与后台进程独立；新环境为空白研究状态，已自动准备模型和研究流程。页面切换不停止研究任务。

模型单次费用上限引用同一共享配置，现有环境和未来模板的默认值为 $1。当前 IT 服务环境每日总预算 $180，其中持续研究 $100，其他预算池 $50/$25/$5；保存时自动更新相关研究授权和准入绑定并保留已用金额。线上回读覆盖 42 份模型配置及 78 个有效预算入口。

ACN、CTSH、EPAM、IBM 电话会均为 4/4、年报各 1/1；当前电话会和年报全文阅读证明仍为零。文档抽取已通过两环境实际 Cockpit 保存切换到 GLM-5.3 Flash，并更新未来创建模板。配置发布与研究结果分别验收，不把服务在线、候选文档或取件成功作为全文读取完成。详见 [多环境运行与验收记录](reports/workspace-runtime-autoprovisioning-2026-09-14.md)。

以下 09-13 各轮开发和未部署记录均为历史记录，当前部署状态以上述 09-14 检查点为准。

## 2026-09-13 当前任务：Cockpit 中文表达与操作体验修复，完成后部署

Owner 已明确恢复本轮 Cockpit 修复并授权自行调试、部署和迭代，先前搁置的高级功能继续搁置。完整原始要求、验收标准、工作树与中断恢复步骤记录在 [Cockpit 修复 Owner 指令](OWNER_COCKPIT_READABILITY_2026-09-13.md)。后续恢复优先读取该文件。
...
发布前最新检查点（21:30 UTC）：`f19e3ad2` 已完成 8,173 项完整测试，1 项失败、0 项错误、3 项跳过；失败为新增中文初稿用途遗漏中文名称，已补齐并通过专项检查。独立 OPS `3c44f5ca` 修复了演练进程混用旧代码的问题，56 项专项检查和独立复核通过。

修正后的复制状态演练发现真实安装问题：现有提取配置固定在有效的路由版本 13，重复初始化却自动改指只增加了其他用途设置的版本 14。正在修复等价路由的既有引用保留逻辑，维持显式路由变更能力；演练的字节一致性要求保持不变。失败测试、安装包和演练证据完整归档，尚未安装 R25。清理空间与修复、测试和部署继续并行，不因清理暂停开发。

以下为本轮逐次检查历史；当前状态以上两段为准。

发布准备最新检查点：模型设置页的登记名称、渠道区分、家族与能力中文标签及选择器已完成。真实浏览器专项覆盖 42 个用途、30 个模型选项，普通正文无 profile、未分类状态、用途及能力代码；顺序、去重、只读控件与原始配置 ID 提交通过，POST 在浏览器内截获，未修改线上配置。组合界面相关 154 项检查通过。

复制状态演练另外发现新增审校用途未在安装初始化前注册，已将四个用途加入既有静态路由登记，真实 production setup 保留用户模型选择的迁移回归和 35 项路由检查通过。预算、模型链与用户选择均保持原值。`c0c95c05` 候选未安装，隔离全量测试在 168.7 秒主动中止，失败演练与日志完整归档；约 1.61 GB 演练副本已清理。下一候选将重新执行复制状态演练与完整测试，再正式安装发布。R24 仍在线。

最终复核补记：`4c0d5a4f` 冻结候选 **8,164 项完整测试通过，零失败、零错误、3 项跳过**，625 个安装包文件与源码一致；语言建议末轮复核另发现模型页内部 profile 编号和少量旧按钮标签，正在补齐，故该候选测试记录保留为通过但已被后续候选取代，未安装。

存量文字已全部完成审校：2,259 条归一化结论的 151 个分块全部通过，三份不完整初稿通过原有一次修订流程恢复，保留原调用与费用记录。实际 Core 只读覆盖检查为 3,086 条可见文本，其中 2,998 条需中文呈现且全部映射，88 条本已通顺中文，待处理和新增模型调用均为零。覆盖收据 SHA-256：`588752cffdb310853282fa81464f1ef18037a829132d63d0de9a12092295dda4`。最终产物仍为 44/45 份审校附件；EPAM 正式纠错与新附件在安装后依次执行。

最新检查点（覆盖下方历史记录）：R24 仍在线，R25 尚未安装。`043868ee` 冻结候选完整执行 **8,127 项测试，零失败、零错误、3 项跳过**；之后新增了真实审校反馈所需的数值等值校验、正文金额显示和后台新增结论审校接线，最终候选将重新执行完整测试。

真实浏览器检查已覆盖 **147 个页面及展开状态、49 条审批、63 页结论和 10 份下载**，五家公司结论全部翻至末页；HTTP、浏览器错误及非读取操作均为零。结论分页和审批中文映射刷新问题已复验通过。已审校界面映射共 1,277 条：原有 529 条、补充审批 20 条、页面摘要及来源说明 728 条。另 2,259 条归一化结论的 151 个分块已完成首轮，其中 7 个待修复后复用缓存；它们尚未合并为线上映射。88 条本已通顺的中文保留原文，未重复调用模型。

后台已接入当前任务可见新结论的自动发现和稳定分批审校，复用同一份付费阶段缓存；中断后即使审校附件已存在，也继续恢复缺失的界面字典。已完成的语言检查不会再次送检。中文正文的美元金额按万／亿美元显示，原文引语、科学计数法与 EPS/ARPU 等小额指标保留合适口径。清理空间和开发继续并行，可用空间约 24 GiB。

冻结前最新复验：修复后的真实候选完成 94 个页面及展开状态、49 条审批、10 份下载，HTTP／浏览器错误与写入操作均为零。五家公司结论已全部翻到末页，数量与总数一致；旧字典已覆盖但因缓存残留的 10 条英文审批正文全部消失。SEC lane 测试清理竞态已修复，连续五轮共 50 项专项通过。接下来重新冻结代码、执行完整测试；本段仍不表示 R25 已上线。复验收据分别为 `dalton-r25-approval-localization-live-ro-v2.json`（`970dd666…a76`）与 `dalton-r25-claims-full-pagination-live-ro-v1.json`（`58c663c7…0ef`）。

最新检查点（以本段覆盖下方较早记录）：R24 仍在线，R25 尚未安装。真实候选页面检查覆盖 93 个展开状态、五家公司 25 个页签、49 条审批和 10 份下载，发现结论翻页游标的嵌套排序键校验错误，以及中文映射晚到时审批卡片未刷新的竞态；两处已修复，正在复验。全部结论已逐页读到末页，五家公司分别为 591、685、981、338、497 条。

存量审校继续使用原有缓存：20 条新增审批文字已通过；728 条页面摘要／来源说明与其余 2,259 条归一化结论正在补做中文初稿、单次 Antigravity 语言检查、大脑修订和独立事实保真核验。它们是系统整理的正文，不把这类英文整体当作原文引语豁免。45 份最终研究产物中 44 份已有审校附件；EPAM 的五处年份纠错仍待新运行时安装后的正式记录与审校。

完整测试的后续记录：`7ae99f38` 执行 8,125 项，唯一失败是旧中文标题断言，已更新；`7ee59775` 执行 8,126 项、0 failures、1 error、3 skips，错误为 SEC lane 测试退出时子进程仍在写临时目录，正在修复等待／清理竞态。两轮均未作为部署通过证据。最终候选需在修复后完整重跑。

OPS `1490d1cd` 的 149 项检查通过；两次真实复制状态演练暴露的配置清单与模型路径改写问题已修复。独立复核确认最终工具绑定 81 个 helper、复制目录 v4，原 provider 验收记录与已授权 host 修复之间有精确校验链。失败演练的大型副本已清理，保留小型日志与完整哈希收据。清理与开发并行，研究数据库和付费缓存保留。

最新复验：首份冻结候选 `b98a9ad7` 完整执行 8,097 项测试，2 failures、0 errors、3 skips；两项分别为旧页面标题断言和三个新模型环节缺中文名称，已修复，相关 55 项回归通过。首轮失败记录与 wheel 完整存于 `foundation-r25-release/candidate-history/b98a9ad7-failed-suite`，不作为部署通过证据。修正后的候选仍须重新冻结并全量验证。

第六轮存量审校已完成：45 份最终研究产物中 44 份、10 个界面文字批次已有完整中文审校附件。IBM 初筛已完成，复用了此前成功返回的初稿，未重复语言检查。EPAM 初筛 S3 的四处财务年份和一处趋势叙述与原始 Claim 不一致，需要新增正式纠错版本，再审校中文附件。严格纠错流程已合入，使用当前任务版本、独立人工决定和重新评审；尚未修改线上记录。 实际 Core 副本演练已通过，重复执行复用同一版本与决定，旧版本保留且仅 5 处文字替换。当前任务 v14 的新评审结果为未通过，原因是缺少过去四个季度电话会纪要；纠错发布与研究阶段是否通过分别记录，不沿用旧任务的通过结论。

本轮末次逐项核对新增修复：下载名使用公司、类型与日期；49 条审批全部显示可读业务详情，原有操作保持独立；研究 HTML 的内部状态、指标名、成本模板问题已中文化，引用编号移入折叠详情；页面各章节资料缺口共用准确的中文显示。只读真实候选验证中，49/49 审批详情可见，五家公司 HTML 指定内部词残留为零。“阅读完整报告”的独立入口已修复，只有该确切版本的有效中文审校附件才显示正文，历史版本也不会借用当前版本附件。

磁盘清理已完成，开发继续；当前可用约 26 GiB。R24 仍在线。下一步为最终冻结、全量测试、复制状态部署演练、安装及持续健康检查，再做真实页面、问答和导出验收。

末轮全字段扫描已补齐研究正文和资料缺口中的已注册内部词，五家公司 HTML 与研究库的正常显示区域均无裸露 snake_case 字段；专有名称和英文原文中的普通单词保持原样。部署输入中的 provider 差异已定位为本轮已授权的错误分类修复，正在补齐从原验收记录到现行文件的验证链。`df3c040d` 候选在启动 93 秒后因上述显示补丁而中止隔离测试，未安装；其 wheel 与失败收据已归档，不计为通过。下一候选重新冻结和完整验证。
...
存量中文发布准备第三轮已结束：55 项产物/界面文本批次、91 个分块，其中 36 项已有完整附件，25 个分块尚未通过。剩余项包括金额/季度等值误判、检查建议的章节或省略号格式、单次调用估价限制，以及主代理误将清理要求理解为暂停后中断的 7 个请求。已修复可明确核对的检查建议格式和部分数值误判，缩减修订 prompt 中不需要重复的原始数据库载荷。7 个中断请求中 5 个已有付费结果、2 个尚未派发；正在实现严格复用原结果与原预算结算的恢复路径，避免重复调用。所有成功与失败证据均保留，尚未把这些附件发布到线上。清理任务已经完成，开发与部署继续推进。

上述 7 个中断请求现已恢复：5 个复用原 broker 结果并结算原 admission，2 个返回待执行状态；执行收据 `model-orphan-recovery-executed-v1.json` SHA-256 为 `3a6d49b81e09814ea462ed41e88ae5c21947f94a366a73096fc3d73e522fc26f`。恢复代码经独立复核和 24 项测试，覆盖结算后中断再恢复、重复请求、配置/路由/lease 身份以及正常 worker 不得跨越恢复预留。第四轮还在等待一份初稿原调用；第五轮明确排除该产物，继续处理其余 54 项，收据独立保存。检查器的有效省略号/唯一章节错标已修复；初稿仅在内容校验次数耗尽后允许一次大脑修订，网络、预算或忙碌错误不会触发另一笔修订调用。

当前代码已用真实只读数据生成五家公司 HTML/XLSX 候选，5 个 Excel 与基线逐单元格语义相同，1,104 个公式未发现静态错误，百分比一位、EPS 两位，负号保留；HTML 通用双语收入标签残留为零。候选收据位于 `/private/tmp/dalton-export-current-candidate-0913/receipt.json`，SHA-256 为 `e6b4f3b5aa3cd10d4b531e7df439df182f7d1c572d4c79175dba045328b98e6f`。这仍是部署前验证。开始冻结候选运行完整测试，最终部署与页面验收仍未完成。
...
实际审阅覆盖新版导航、五家公司 25 个页签、5 张模型表、结论列表、49 条审批、旧版 5 页与文档分页，以及五家公司 HTML/XLSX 下载。全量摘录见 [Cockpit 页面文字全量摘录](</Users/everflow/Documents/Dalton Cockpit 页面文字全量摘录-2026-09-13.md>)。会产生写入的按钮仅核对源码文案，未把条件模板当成真实提交结果。

内容质量尚未通过：英文异常和内部标识直接展示，中文存在病句及生硬措辞；EPS 被错误按百万缩放；“今日事件”未限定日期，ACN 事件还出现其他公司内容；Ask 对 DXC/IBM 模型归属表述错误；旧版 4 份待抽取文档中 3 份打开返回 HTTP 400。这些问题仍待修复，不因已完成部署而计为通过。高级能力的独立分支工作保留并暂停。

## 2026-09-13 最新 owner 优先级：先发布正式可运行 demo

**R24 已安装并完成正式发布。** 冻结 runtime `7025`、OPS `2790` 的完整测试共运行 **7,949 tests，0 failures/errors、1 skip**，另有 **134 项 OPS 检查**通过；608 个运行文件、17 份模型配置、14 步复制状态演练和部署前 preflight 均已通过。安装退出码为 0；同一 controller 完成 45/45 健康采样，持续 669.670 秒。真实 UI 在 3.382 秒通过，一次真实 Ask 在 62.214 秒完成并返回带引用结果，五家公司 HTML/XLSX 导出与回放通过。正式 publication 已写入并验证。

Owner 将当前范围收敛为“优先开发没有就跑不起来的内容，然后部署”。当前正式发布版本为 **R24 runtime `7025`、OPS `2790`**。R23 的 UI 成功与 Ask 路由失败、R22b 安装失败与健康回滚、R22c 运行通过的历史证据均保留。

R23 真实 HTTPS 页面验收已通过：首屏 3.587 秒，13 秒自然轮询最大并发 1；五家公司模型与研究产物接口均成功，ACN 实际浏览器 HTML/XLSX 下载成功，五家公司独立两次导出语义回放均通过。曾残留两天的开发 QA proxy 持续制造请求积压，已按身份精确关闭并清理对应旧测试 session；没有重启线上服务或关闭用户浏览器。

**R23 的实际 Ask 测试曾被模型路由拒绝。** 原因是问答复用了 extraction 配置，而 Ask 属于 brain tier；配置缺少该链允许的模型和 credential slot，且通用默认单次 $0.05 无法覆盖约 104.6k 输入。R24 改为使用现有研究规划配置，并为交互实例设置 $2 默认单次上限；已有显式预算优先，现有 $100 日预算与 17 份模型配置不变。R24 的一次新 Ask 已真实完成；这项验收证明交互路径可用，不表示全部研究内容或十题质量目标已经完成。

[Demo 使用说明](DEMO.md) 列出了交互研究台与公开只读监督台的不同入口；[R24 发布验收](reports/demo-release-r24-2026-09-13.md) 记录本次真实验证及边界。

本次只推进正式 demo 必需项。模板 registry/select 在独立分支完成 `8e66ee01`、31 项相关测试通过，保留候选。Conviction v2、回报归因和其他高级投资能力暂停；Wiki 路径修复、参考模板导入及合成双工作区工具位于后续集成分支，不属于本次 demo wheel。

## 2026-09-13 vision / owner 指令复盘

完整范围与剩余任务以 [最新覆盖复盘](reports/vision-owner-coverage-refresh-2026-09-13.md) 为准；下方历史检查点保留原日期和结果，不覆盖当前发布状态。

Guidepoint 本地 MCP 已接好，当前受 mission `not_connected` 阻挡；OpenClaw 公司 Wiki 的 skill、向量库与资料均存在。历史资料目录已建立，桌面 STRL / ecommerce 样例保持原样。

尚待开发或验收的高级部分包括 Conviction v2、持有期回报归因、买方预期来源 authority、结构化情景/观察跟踪、模板版本选择集成、真实多工作区与最终人工验收。这些缺口没有因发布 demo 而计为完成。三个 GPT-5.6 Sol 子代理按最新优先级并行处理必要修复、独立核验和发布准备。
...
**实际阅读进展：EPAM 年报 35 个窗口全部正式成功，连续覆盖 419,086 字符，已生成完整阅读证明。** 只读核验已逐窗核对源文件、上下文、WorkOrder 与正式结果哈希。其余历史错误 dismiss 不会自动冒充人类重开；提取内容质量和 Dossier 发布另行验收。

Integration 已修 Dossier 生成/验证调用缺失 mission 内容哈希及 verifier 请求标识不匹配（203 tests），空闲 Scheduler sweep 不再无谓争抢写锁（71 tests），可持久化选择失败域名冷却计划（17 tests），以及建模规格提示词与字段长度要求冲突（49 tests）。这些后续修复尚未部署。Sol 并行补齐 Dossier 配置/恢复身份、真实模型失败证明与逐调用重试计费、定向年报检索→分析→独立验证→候选暂存。

电话会清单已完成新的只读核对：有效季度仍为 ACN 0 / CTSH 1 / EPAM 2 / IBM 0 / DXC 4；旧提取记录仍在。候选页面新增“历史已提取（全文待核验）”，避免新全文证明口径让旧成果看似清零；79 tests / 1 skip 通过，尚未部署。AlphaEngine 取文前相关性筛选与缺季度补搜正在独立修复。

**Next step：** 完成真实执行路径复核后冻结下一版，执行全量、wheel、当前状态副本演练并部署；重新验收稳定性和实际 Dossier 0.3。失败域名冷却配置随后在同一维护窗口启用。高级投资判断仍保留胜率/赔率/市场隐含预期的独立维度，不把方法论逐项塞进所有 prompt；基础能力完成后再推进。最终视觉重构、基金参考文件导入/交付格式、多分析师 workspace 继续按 owner 顺序后排。详见[基础执行与阅读进度登记](reports/foundation-runtime-and-reading-closure-2026-09-11.md)。

## 前一检查点（2026-09-11，03:42 UTC）
...
新 PID 69511 于 03:18:30 UTC 启动，初始健康在安装等待窗口内通过；03:20:39 起按默认健康门槛观察 45 次，必须同一 postdeployment 进程且至少 660 秒才记运行通过。当前接受状态明确为 `installed_bytes_verified_runtime_pending`，不以首批健康代替完整观察。

后续基础性能改进已合并并独立审查：Dossier 同一次操作共享 Claim snapshot，12 单元新旧输入完全相等，读取次数从十余次降为一次，无跨 tick 缓存。159 项集成检查通过，冻结 R8 `86ad4dc` 全量测试进行中，wheel 556 文件/3 JS 已通过；真实状态副本性能比较正在执行，尚未部署。高阶投资能力、最终视觉、多 workspace 激活继续后排。

**Next step：** 先完成 R7b 运行/真实证明验收；R8 同版全量、数据副本演练及实测输入等价通过后再部署，复核稳定性。现有来源许可与人类研究裁决仍独立保留。

## 前一检查点（2026-09-11，03:03 UTC）
...
修复只让副本检查在真实 SQLite writable owner 存活期间打开 strict reader，不造空 sidecars、不改只读保护；101 项演练回归通过。发布 stager 另经独立故障注入验证：回滚只恢复仍属于本次写入的精确字节，并发编辑保留且明确失败。已合并所有基础修复和报告，准备以 R7a 同一冻结重跑验收。

**Next step：** R7a 全量、wheel、状态副本演练均通过后部署；实际新 Dossier0.3 独立验收工具已备妥，部署前基线诚实为 `pending_no_v03_records`。继续跨调度周期核验健康，基础完成以前不扩展高阶投资能力。

## 前一检查点（2026-09-11，02:40 UTC）

**下一基础候选 R7 已合并，准备冻结验收；live R6 的运行稳定性仍保持未通过标记。** 完整页面投影和 render/publish 已移出心跳线程，后台严格单飞串行、源变更自然追赶、失败可见；投影活跃时不会重试旧插件。`--once` 仍等待完整产物，shutdown 等待自己拥有的后台任务结束。未放宽健康门槛。独立 service 审查通过，Dossier/路由/服务合并 292 项通过。

模型路由仅对自有可写磁盘库迁移 WAL；caller-owned、内存与只读连接保持原所有权。另将 Scheduler 写事务内的过期 lease 查询改用已有索引排除历史事件；实际规模查询约 344ms→10ms，保留租约与 5 秒锁等待合同。66 项 Scheduler/service 集成通过。研究材料的逐 Claim 无索引证据扫描改为有界批量读取，输出语义不变；一致 SQLite backup 上四家公司 48 单元/1,713 条资料已重建，无形状失败。重建总计 118.4 秒，重复 ClaimIndex/view 构建的进一步优化仍单列后续，不以这项局部改进冒充整体性能完成。

Dossier 0.3 证明、旧版增量兼容及这些并发修复统一进入本次冻结。两份较旧部署备份经每个文件及元数据核对、归档持久化和无句柄复查后压缩，保留可恢复归档与校验记录；R5/R6 展开备份仍保留。另一会话 DeepSeek diff 未改变。

**Next step：** 同一冻结并行全量测试、wheel 逐字/文件集验证、当前状态副本演练；通过后按现有授权部署，核对配置、mission、实际 Dossier 新证明与跨调度周期健康。高阶投资方法与最终视觉继续后排；资料与人类 checkpoint 不被开发或部署授权替代。

## 前一检查点（2026-09-11，02:31 UTC）
...
01:41 产品快照有 12 条 EventJudgement，均通过正式 producer、独立 verifier、路由与 mission 绑定检查。ACN 10-K 已成功结算，CTSH 已启动，EPAM/IBM/DXC 待正常队列推进；01:44 ACN 已自然生成新规格，现金流输入包含 13 个连续季度且无 gaps，forecast action 正确为 new_specification，尚待正式新模型。没有强制重预测或清除持久拒绝。

Dossier 验收仍有缺口：ACN 是已发布的增量版本，仅 3 个单元具备当前正式证明，完整性 rubric 未通过；EPAM guidance_style 的历史 producer 无法通过重建当前输入唯一定位。正在分别修复部分发布的状态表达、调查历史单元证明的回放合同；不把增量 authority 存在计作完整档案。

**Next step：** 并行完成 Dossier 状态/来源证明修复、年报→规格→现金流模型→季度/年度 Excel 的实际验收，并复查基础预算/模型配置一致性。高级投资方法仍后排。部署授权已具备；来源和人类研究裁决继续独立保留。

## 前一检查点（2026-09-11，01:37 UTC）

**基础版本 `598f2859c59775882f6d85482db09407c8568abd` 已正常部署并完成运行核验。** 同一冻结 6,829 tests / 1 skip；556 个运行文件与 wheel 一致，1,841 个保留源码文件与精确 Git archive 一致。部署 exit 0，15 DB 备份及恢复校验通过；控制器同一 PID 51807 连续三次健康检查通过，心跳年龄 0.95 / 5.25 / 14.81 秒。备份为 `deploy-backup-20260911T013344Z`。

12 份模型选择、预算和 credential 准入保持；原 mission v14、broker 容量配置以及精确历史未决调用记录均未改变。R4a PID 复用导致的部署阻塞已通过本次真实 drain 验证关闭，未信号任何无关系统进程。年度队列、档案确定性校验、Excel 公式、现金流输入及逐操作数 authority 证明均已进入 live。

**Next step：** 实際产物验收正在并行进行：10-K 成功结算 → 新规格通过 → schema 0.2 财务模型追加 → 季度/年度与现金流 Excel，另逐单元检查 Dossier 和正式 EventJudgement。安装成功不替代这些产品条件。高阶投资方法仍后排；来源和人类研究裁决保持原边界。

## 前一检查点（2026-09-11，01:25 UTC）

**基础修复已统一冻结为 `598f2859c59775882f6d85482db09407c8568abd`，live 保持原稳定版本。** 包含年度报表队列、Excel 漏公式修复、档案校验修复、可配置启动等待，以及新完成的 typed cashflow 输入/精确季度差分 authority 证明和部署 PID 身份核验。234 项集成检查通过；新 wheel 的 556 个源码文件逐字一致、3 处 JS 通过。完整测试与当前状态副本演练在并行进行。

PID 修复已独立审查：从同一 ticket 文件描述符取字节及稳定元数据，进程出生时间须处于启动请求与票据写入的真实区间；慢启动保留，明确复用排除，不可证明时保守等待。现金流旧 schema 0.1 读取字节不变，0.2 操作数缺项/未知项/类型异常及伪季度差分均明确拒绝。另一会话的四份 DeepSeek 改动保持原字节。

**Next step：** 同一冻结全量、wheel、副本演练通过后再部署，并验收年度报表、现金流实际产物、档案和 Excel。业务来源批准和人类研究裁决保持原边界；高阶投资能力仍后排。

## 前一检查点（2026-09-11，01:20 UTC）

**继续先补基础闭环，live 仍为 `2e94acd` 且已恢复健康。** R4a `c0295d8` 同版全量 6,810 tests / 1 skip、wheel 556 文件、67 schemas / 40 entries / 0 escaped 副本演练均通过。部署在替换代码之前发现旧 research-plan ticket 的 PID 已被无关 macOS geod 进程复用，原先只看 PID 是否存活的 drain 因而误等。仅终止本轮只读等待程序后，部署脚本恢复原服务；未替换 runtime、未改 ticket、未重启 gateway、未触碰系统进程。

现金流 typed inputs、累计季度差分及逐操作数报表 authority 证明已合并并独立审查，164 项集成回归通过；实际旧 schema 0.1 模型读取字节与 hash 保持。IBM 数据仍缺由 10-K 补齐的 Q4，因此尚不声称完整 FCF。当前并行修复进程身份判断，并补齐新现金流合同的异常输入校验；这些都是新一轮基础冻结的范围。

**Next step：** 进程身份修复交叉审查后合并，连同现金流冻结验收再部署；检查实际年度采集、12 单元公司档案、正式事件和 Excel。五家公司已有 44 条正式预测线，5 条事件判断已验收；CTSH 来源额度、行情来源许可和人类裁决保持原边界。高阶投资方法继续后排。

## 前一检查点（2026-09-11，01:04 UTC）

**基础验收继续；live 保持 `2e94acd`。** R4 全量 6,809 项中有 1 项失败（另 1 skip）：安装器新增的 zsh 数字模式未通过既有 bash 语法检查。已改为兼容语法、先限制数值长度再按十进制解析；17 项聚焦检查通过，独立审查通过。新冻结 `c0295d89fbf165b7814c16eab459aa862578eb29` 正在重跑全量；R4 失败证据保留，未执行部署。

五家公司正式 forecast models 共 44 条已验收；最新只读产品检查另确认 5 条正式 EventJudgement 的 producer/verifier、路由、mission/event 绑定和模型独立性通过。ACN Dossier 尚无正式产物，EPAM 最后三个单元仍待完成。下一版年度采集的五家公司前置授权均有效，没有额外人闸；待部署后才会开始年报队列。IBM OCF/CapEx 实际数据已存在，缺口在模型输入消费者合同，正隔离修复并独立审查。

**Next step：** 新冻结全量、wheel 与副本演练验收后部署，检查实际年度报表、档案和导出；现金流修复单独验收，完整模型不以已有收入预测替代。高阶投资能力继续后排。

## 前一检查点（2026-09-11，00:57 UTC）

**基础能力优先；五家公司当前正式财务预测模型已全部通过在线只读验收。** ACN、CTSH、EPAM、DXC 各 8 条，IBM 12 条，共 44 条正式预测线；规格、实际输入、模型和预测线哈希一致，filing proof 可回放，economic invariants 无拒绝。Live 仍为 `2e94acd`，不把尚未部署的导出修复算作线上能力。

下一基础修复已冻结为 `fdc61e9a140993d5c2af427e53d98ed8eede7845`：年度与季度报表分别调度并严格绑定 observation form、保留可配置采集深度；修复 IBM Excel 漏掉的 60 个公式及审计页可读性；修复有来源的完整日期和历史分析师目标价变动被误拒；安装器健康等待改为可配置有界期限。集成回归 260 项通过；556 个 wheel 文件逐字匹配、3 处 JS 通过；当前状态副本演练 67 schemas / 40 entries / 0 escaped。完整测试仍在运行。

实际 EPAM Dossier 为 9/12 单元，事件正式判断仍待验收；年度报表尚待新调度采集。来源许可、人类裁决和 CTSH 滚动 24 小时限额仍按原合同执行。高阶投资方法、视觉扩展不抢占这轮基础闭环。

**Next step：** 同一冻结全量通过后部署，保留模型/预算配置和已经应用的 broker 容量 8，再检查实际年度报表、Dossier、事件与 Excel 产出；及时 commit/push。

## 前一检查点（2026-09-11，00:28 UTC）
...
**基础版本 b5ee132 已成功部署并完成运行核验，实际产物继续验收。** 6,770 项完整测试通过（1 skip）；555 个安装运行文件与接受 wheel 一致，保留源码 1,828 个 tracked files 与冻结 commit 一致。控制器同一 PID 29734 连续三次健康通过，心跳年龄 5.28 / 1.33 / 0.49 秒。原 mission v14、12 份模型选择/预算/credential 准入保留；部署日志正常结束。备份为 `deploy-backup-20260910T233849Z`。

首份新模型规格已经产生：IBM 的 cost-template metadata 在正式 authority 回读成功，真实 input builder 通过。其余四份等待有界自动调度，正式 forecast model 暂仍为零，不将只读纯计算当作已发表模型。Dossier/事件已启动新的正式工作；仍须检查 verifier 和最终 authority 记录。

**Next step：** 沿真实产物检查剩余基础阻塞；并行复核模型规格首公司挂起是否挡住后续公司，以及价格/预期/日历来源未获许可却显示“全部 current”的状态误报。资料与人类 checkpoint 保持原门槛，高阶投资能力继续后排。CTSH 额度详见 [滚动额度审计](reports/ctsh-alphaengine-rolling-quota-audit-2026-09-10.md)。

## 前一检查点（2026-09-10，23:39 UTC）

**b5ee132 完整验收通过，已开始按既有授权部署。** 同一干净冻结 6,770 tests / 581.148s / 1 skip；wheel 555 个运行文件逐字一致、3 处 JS 检查通过；当前状态副本 67 schemas / 40 entries / 0 escaped。正式部署前 12 份模型配置已留存快照，脚本与接受版本哈希绑定通过，磁盘预检需要约 3.54 GB、可用约 5.35 GB。此时仍不能称实际研究产物验收完成。

CTSH 只读复核确认当前 AlphaEngine trailing 24h 为 130/130（search 22、document 108），首次释放为 2026-09-11 16:26:44 UTC / 12:26:44 EDT；有效电话会仍为 1/4。Spend 参与 planner 输入哈希，且无变更时也每小时重查，不会永久缓存等待计划。没有修改额度、治理或资料门槛。

**Next step：** 完成安装，核对实际源文件、原 mission v14、模型选择/预算/credential 和连续健康心跳，再检查真实新规格、forecast model、Dossier 与事件判断。仅为实际可复现的基础阻塞继续修复，高阶投资能力后排。

## 前一检查点（2026-09-10，23:29 UTC）

**最终基础候选已冻结为 b5ee132，完整回归正在运行；live 仍为 d297c0f。** 前一冻结 d071f8d 的 6,728 项完整测试通过，但实际 Dossier 诊断随后发现 verifier 丢掉 producer 已看到的来源 period，因而将合法日期判成无来源；已修复投影并绑定新核验合同，110 项独立回归通过。

新冻结还包含精确授权的 child 受控重入：旧摘要原字节与哈希写入 owner-only append-only marker，claim/spawn 同一单飞锁，短写/写失败在启动子任务前停止，未 enqueue 崩溃也不会无限重试。模型 request 基础身份、预算、真实内容拒绝、人类审批边界均不放宽。合并回归 266 项通过，r2 wheel 555 文件逐字一致和 3 处 JS 检查通过，67 schemas / 40 entries / 0 escaped 副本演练再次通过。

**Next step：** 等同一最终冻结全量通过后一次部署，再核验真实 Dossier、事件判断、规格元数据与正式预测模型。CTSH 的当前 AlphaEngine 滚动额度及实际来源阻塞正做只读复查。基础闭环优先，高阶投资方法继续后排。

## 前一检查点（2026-09-10，23:20 UTC）

**基础修复已冻结为 d071f8d，完整回归正在运行；尚未部署。** 冻结前 18 个改动测试模块共 516 项通过，wheel 554 文件逐字一致、3 处 JS 检查通过，同版当前状态副本演练 67 schemas / 40 entries / 0 escaped。成本模板持久化漏项、五条基础调度路径的公平性、ZeroBase 路由误分类、WAL apply 与部署空间预检均在本次范围。详见 [基础后续版本验收](reports/foundation-followup-release-2026-09-10.md)。

真实快照仍为四份 DebateMap、零正式 forecast model；五家公司旧规格缺成本模板元数据的原因已用只读实际 input builder 复现。新规格按新 task contract 追加，旧记录/哈希不重写。受控恢复的新 child reentry 补丁仍独立开发，需先补齐保留旧摘要的持久尝试记录与单飞边界，不抢入当前冻结。

**Next step：** 同一冻结完整测试通过后按现有授权部署，再验收真实规格、模型输入及正式研究产物；继续完善受控重试审计。人工来源/研究裁决仍保留，高阶投资方法继续后排。

## 前一检查点（2026-09-10，23:05 UTC）

**基础能力优先，live 仍为已验收安装的 d297c0f。** 只读实际产物快照已增至 4/15：ACN、EPAM、IBM、DXC 均有当前 mission 和 evidence fingerprint 匹配的 DebateMap。Dossier、事件判断与财务预测尚未验收，不把测试或服务健康计作产品成功。CTSH 仍缺通过的 Initial Screen 和完整来源前置。

事件调度的逐使命/逐事件组公平性、持久失败和精确子任务绑定已集成；最新组挂起时仍可处理同公司更早的组，组内容变化拒绝旧选择，其他新事件不使选中组失效。Dossier 已按公司实际有界输入生成指纹。新故障摘要保留经过正式 WorkOrder/result 哈希校验的模型失败关联；未知旧关联不猜测。部署空间预检、WAL apply 生命周期与预测校验合同升级也已集成，尚待新冻结全量、wheel、副本演练。

Deep Insight、Investment Memo、财报窗口的相同调度缺陷由 Sol 并行修复并交叉审查。审查发现财报窗口前 8 项截断可藏住后续挂起外任务、无摘要子进程失败被静默记为 quiet，以及模型配置变更未释放 hold；正在补齐，未部署这些候选。

**Next step：** 完成交叉审查与回归，冻结精确版本，完成全量/wheel/当前状态副本演练，沿用已有部署授权更新 live，再检查真实基础产物。人工来源/研究判断仍保留；高级投资判断方法、后续设计扩展不抢占基础闭环。

## 前一检查点（2026-09-10，22:33 UTC）

**基础版本 d297c0f 已安装，实际产品验收继续。** 同一冻结版本全量 6,671 tests 通过（1 skip），wheel 与 553 个 live 运行文件一致，保留源码 1,806 个 tracked files 与接受 commit 一致。受控 host transport 的三份文件也通过精确哈希核验。12 份模型配置的选择、预算和实际 credential 准入保留，mission v14 原签署不变。

安装器最后的 health 检查遇到磁盘耗尽失败；控制器因 projection/heartbeat 写入 ENOSPC 重启。仅清理已完成临时演练副本，保留报告与部署备份，释放到约 3.8 GiB 后服务自行恢复。曾有一次磁盘 I/O 导致心跳过期，随后同一 PID 连续恢复 10–13 秒心跳；不通过放宽 health 阈值掩盖问题。部署前空间预检已提交 3fafaff，尚未部署。

只读真实产物审计现在为 1/15：ACN DebateMap 带当前 mission/input 绑定，其余尚未通过验收。CTSH 财务维度错误来自 15:43 的旧运行摘要，当前保守校验已拒绝把重复 contextual facts 当成独立分部；正在检查旧失败为何尚未触发一次新合同下的重算。19 条旧 host 错误已追加费用保守更正和 hash-bound operator recovery，不直接发模型请求，未删除任何旧任务或结果；预算按原 reservation 保留风险。

**Next step：** 验证恢复后的真实独立核验产物，修复基础任务的旧合同失败缓存，补齐部署磁盘预检与 apply 阶段 WAL 生命周期回归；继续 commit/push。人类研究/来源裁决保留在私有复核包，高阶投资方法继续后排。

## 前一检查点（2026-09-10，22:06 UTC）

**基础能力验收继续；live 仍为 c84d076。** 第一轮候选 925e50c 全量 6,661 项中有 19 项失败；随后 e9d06c2 的重复回归已主动中止，两者均未部署。失败原因和修复记录见 [持续进展](reports/autonomous-closure-progress-2026-09-10.md)。

新冻结 `d297c0f` 正在完整回归。前置 225 项通过，此前抽取/模型 181 项、合同/演练 211 项通过。此版补齐 issuer 证明测试样本、SEC v3 proposed 演练种子、v0.2 model spec 测试合同；新模型规格可在原 company model chain 追加新版本并保留旧预测；Dossier 已撤回 Claim 在查询限额前排除，全退役材料零模型调用。所有修复经独立复核；费用和模型选择仍沿用可配置合同。

**尚未完结：** 19 条尚未撤回的错公司 Claim（EPAM 6、IBM 8、DXC 5）及 IBM 旧 screen 的两条引用已整理为私有精确哈希复核材料，未擅自撤回、重开或签署。新调用被阻断/重试可用、服务 health 或测试通过，都不代表实际研究产物已经验收。高级投资判断仍严格后排。

Next step：等同一冻结代码的全量、wheel 和新副本演练全部通过，按已有授权部署并核验真实产物。需要人类裁决的历史资料/来源事项单独保留；不用它们阻挡其余已授权基础开发。

## 前一检查点（2026-09-10，21:43 UTC）

**Owner 再次确认基础能力优先。** 候选 `925e50c` 已在独立不可变工作区启动完整测试；同版当前状态副本演练 67/67 schemas、40 tick entries、0 escaped 通过，wheel 核验并行进行。候选包含跨 tier credential 准入、transcript fallback 的逐调用预算绑定、未知费用保守记账与追加更正、WAL 锁竞争修复、资料 issuer 校验、显式模型 revenue anchor 和精确受控 host 故障恢复。尚未部署，不能将这些实现写成真实研究产物验收通过。

最新只读审核还发现 Dossier 生成提示没有声明解析器的六条 gaps 上限，导致实际生成后被拒；已独立派修，严格解析器不放宽。ACN/EPAM/IBM 的历史 Initial Screen 通过与当前资料前置不一致，精确绑定的复核包已完成，留给人类裁决。CTSH 当前有效 earnings calls 为 1/4（FY2026-Q2），旧不完整审核的 0/4 已另附勘误保留历史。

**Next step：** 完成冻结版本验收与 Dossier 合同缺口修复，必要时重新冻结；按已有部署授权备份、部署并校验实际运行与真实产物。高级投资判断方法继续后排。模型选择、预算和原 mission v14 签署必须保留；来源批准和人类研究裁决不由部署授权替代。

## 前一检查点（2026-09-10，21:12 UTC）
...
**预算与运行恢复版本 `38721e4` 已部署。** 同一冻结版本 6,323 项测试 / 530.413s 通过（1 skip），wheel、当前状态副本演练及部署后 518 个运行文件逐字校验通过，服务 health 全部正常。沿用 owner 已签 mission v14，没有重签。首次产物检查仍为 0/15，正在检查新运行的具体结果；详见 [冻结验收与部署](reports/v14-product-recovery-acceptance-2026-09-10.md)。

下一批：Cockpit 审批草稿被刷新清空、提问返回页面后答案丢失/历史任务不继续刷新两项功能修复已提交并经真实浏览器离线验证；Investment Memo 起草/核验/人审和前置模型阶段正在 Sol 并行开发、交叉审查。前置 gate 的两个宽松判定问题已被拦截，未部署。

**最新视觉要求：** 暂停零碎视觉修补；功能完成后完整重构 Cockpit 的视觉和页面逻辑，以苹果式浅色、精致华丽的方向创造性呈现研究工作流。响应式补丁保留在独立分支，暂不合入。详见 [页面检查与最终设计要求](reports/cockpit-browser-review-2026-09-10.md)。

## 历史检查点：签署与产物验收（2026-09-10）
...
**该历史检查点的 next step（已由顶部覆盖）：** 冻结预算与 post-signature 修复，完成全量测试、wheel 和当前副本演练后更新部署包并部署；沿用已签 mission v14，对 ACN、CTSH、EPAM、IBM、DXC 的 dossier、DebateMap、事件判断做真实产物验收。随后系统检查 Cockpit 页面与交互，按 [owner 期待差距复核](reports/owner-expectations-gap-refresh-2026-09-10.md) 推进真实缺口；Investment Memo 闭环优先，Excel 导出仍后排。当前 15 项目标产物均缺失；stub launch 不算产物成功。SEC 8-K plan 和 DeepSeek 家族声明仍是单独的人工作业，未执行。退役 Agenda 若重启使用，需先明确对齐旧策略 pin。
