...
Current checkpoint (2026-09-11 07:35 UTC): R9c `c29b3e22808cb900b458eeb00d9e5447b72f5db6` is deployed and verified (6,960 tests / one skip; 561 runtime files; 45/45 healthy samples over 676.57 seconds). Three actual ACN Dossier 0.3 versions passed persisted producer/verifier proof checks. Web failed-host cooldown was activated at 07:21 UTC with exact configuration preservation and a healthy replacement controller. Following the owner's additional instruction, the next candidate keeps failed hosts quarantined across window expiry, serves healthy sources first and permits only one low-priority recovery probe; failure extends the delay. Both AlphaEngine earnings-call and sell-side candidate selection now run before acquisition with real subprocess tests; 188 combined tests passed. These next-release changes are not live yet. Directed annual-report Writer/CLI execution and real runtime configuration setup are under final independent review.
