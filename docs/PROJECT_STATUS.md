...
## 当前检查点（2026-09-10，19:49 UTC）

**Live 仍为已验收的 `7bdccd8`，Mission v14 人类签署不变；新一批正在最终联合验收。** Office 完整导入、HTML/XLSX 及 Cockpit 下载已经部署。基金 XLSX 新补年度在左/季度在右、可证明的四季流量合年、百万单位显示，底层值与公式不变；123 项相关测试通过并完成真实重算/渲染。

CTSH 真正阻塞包括自动提取仅接受单模型的旧假设，与 Cockpit 多模型回退链不兼容，已修复；所有视图失败现在如实报告失败。季度 gate 要求最新已识别季度向前连续四季，排除普通行业会议。当前材料为 1/4，锚点 FY2026-Q2，缺 FY2025-Q3、FY2025-Q4、FY2026-Q1；不是等满 24 小时就会自动齐备。

受控核验错误已定位为 `REQUIRED_CONTROLS_UNAVAILABLE`。第一次候选虽有 6,463 项通过，但真实 catalog 验收发现 rateCard schema 不匹配，**未部署且不算接受**；`06d35ef` 已按 broker 的实际十进制字符串价格字段/有效期修复，真实公开配置投影得到 23 profiles、2 个 controlled profiles。显式 verifier 选择修复包只改路由，不改变研究签署，正在副本验证完整性/回滚/停机检查。

多分析师 workspace 的身份、路径、进程、端口、独立 Cockpit、不可变 release、只读 fleet 索引以及模型/连接器共享账户额度已实现。跨策略版本和凭据轮换维持账户总额；不确定的已发调用保留费用与并发占用。连接器过期前置预约恢复 77 项通过。最终审查发现模型预约 mutation 缺所属账户检查，正在修复并加双账户测试；**尚未宣称最终验收，也未创建新的真实分析师或迁移旧数据**。

Next step：集成最后所有权修复，冻结同一 commit 完成全量/wheel/新 live 副本演练；按既有授权部署并应用可回滚的受控 verifier 选择，观察真实自动提取和核验结果。新来源与研究 checkpoint 人工裁决仍保持待人类处理。
