...
Current checkpoint (2026-09-12 21:03 UTC): R18b frozen `3855c1e3` is installed, but its continuous health acceptance failed (43/45 healthy samples); owner publication remains R17a. The same clean freeze passed 7,796 tests (zero failures/errors, one skip), wheel608/three-JS byte verification, and copied-state rehearsal14 steps/42 ticks/zero escapes. Deployment finished at 20:42:51 UTC with exit 0; installed608 files/17 model configs, broker tree and host patch were verified. The 670.948-second observation preserved two outbox writer-RPC timeout samples; the same controller later recovered. Fair planner/outbox serialization and an evidence-backed IBM direct-pretax bridge rule are in development for the successor, with no health waiver or old-response acceptance. R18a's failed 7,794-test run remains immutable and was never accepted/deployed. R18b fixes its delayed-search execution-time accounting with UTC windows/free replay/interrupted-registration holds, and adds compact JSON guidance without changing the 6000-token post-response ceiling. The reviewed runtime/broker/ops fixes remain as previously documented; explicit verifier control guards do not claim hard output control for IBM's producer path. An independently reviewed task-identity audit improvement is integrated only for the next candidate as `4f2fea0a`; it is not in the frozen/deployed R18b source. No model preference, uncertain reservation or source/human authority is changed. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
