...
Current checkpoint (2026-09-12 20:25 UTC): R17a remains the deployed verified runtime. R18a frozen `85dd42c5` completed 7,794 tests with two failures, zero errors and one skip; it is not accepted or deployed. Both failures reproduce independently: search invocation timestamps inherit an old request creation time, excluding a newly executed delayed request from trailing-24h accounting. The fix is integrated as `de6402da` and independently approved: first execution time, UTC-bounded accounting, immutable free replay and an explicit interrupted-registration hold. All 95 related integration tests pass. R18b is ready to freeze for complete acceptance. R18a wheel608/three-JS and copied-state rehearsal14/42/zero escapes passed, but do not supersede the failed suite; immutable failure evidence is retained. The independently reviewed compact model-spec JSON guidance is integrated for R18b, preserving all semantic fields and the 6000-token post-response acceptance ceiling; reduced character formatting is not a measured token saving or a provider hard cap. The release also retains the reviewed downstream failure authority, terminal lane fairness/holds, exact local no-send evidence, explicit verifier transport controls/fallback, financial role/unit guidance, and broker/host deployment proofs. No budget, model preference, uncertain reservation or source/human authority is changed. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
