...
Current checkpoint (2026-09-11 18:09 UTC): R13c `4d459e54` remains the verified live release. R14 runtime is frozen at `42d37ae558556dd73c25b94abf72b18c18d4f76c`; full suite and wheel build run in parallel, with separate reviewed ops preparing actual-state rehearsal. R14 contains configurable 1 MiB model frames and pre-send accounting, per-search OpenClaw provider following on both Python and host broker, signed provider guard and provider-bound journal identity, and narrowly proven recovery of old provider-contract failures after selection changes. Integrated targeted validation: 212 Python tests and 19 broker tests passed; separate recovery review: 27 tests passed. The live Gemini selection is verified, but new broker code and search recovery are not yet deployed. Normal revenue sensitivity joins are verified for 5/5 companies; EPS is unavailable. Company-specific statement structure foundation `5292ae71` is isolated from R14 and not yet connected to production forecasts. Two Sol agents are now implementing model-spec structure proposals/persistence and actual formula-DAG forecast consumption, including correct diluted-EPS numerator, unit and fiscal-window semantics. Root removed 105 obsolete R12/R13b scratch SQLite files (5,935,013,888 bytes) after no-open-handle and hash checks; all configs/logs/proofs, R13c scratch and deployment rollbacks remain. Next: finish R14 actual-copy and package acceptance, deploy/observe, prove normal planning and search recovery, then integrate and validate company-specific modeling. Keep the source-signature review packet unsigned until owner action; preserve current Cockpit style.
