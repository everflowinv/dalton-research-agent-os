...
Current checkpoint (2026-09-12 19:27 UTC): R17a remains deployed and runtime-verified (7,730 tests / one skip; 45/45 health samples). Continued development found and corrected a material error in the prior product diagnosis: IBM's synthetic `BUDGET_REFUSED` / `invocation:not-started` did NOT prove no send. Exact broker journal evidence shows a completed gpt-6-astra response, reported cost $0.80651, and 6,843 output tokens against the Work's 6,000-token ceiling. The adapter correctly rejected the breach, but Cockpit collapsed this post-call failure into a pre-dispatch-looking budget refusal and discarded dispatch context. The R17a product-budget-v1 no-send conclusion is superseded by immutable product-budget-correction-v1 evidence; runtime acceptance is unaffected. Three GPT-5.6 Sol agents are fixing failure authority, model-lane repeat launches/fairness, and checking actual provider-control forwarding. Root is tightening exact no-send audit proofs and preserving the correction. No budget cap increase, unknown-send refund, or blind paid retry is authorized. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
