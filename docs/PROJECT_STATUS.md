...
Owner 将当前范围收敛为“优先开发没有就跑不起来的内容，然后部署”。当前已发布基础版本为 **R23 `3e4e8682`**：完整 **7,945 tests、0 failures/errors、1 skip**，608 个运行文件及 17 份模型配置核验通过，14 步复制状态演练通过，45/45 持续健康采样通过并正式发布。R22b 安装失败与健康回滚、R22c 运行通过的历史证据保留。
...
**正式交互 demo 尚未交付：实际 Ask 测试被模型路由拒绝。** 原因是问答复用了 extraction 配置，而 Ask 属于 brain tier；配置缺少该链允许的模型和 credential slot，且通用默认单次 $0.05 无法覆盖约 104.6k 输入。R24 最小候选正在修复 Cockpit 安装绑定，使其使用现有研究规划配置，并为交互实例设置 $2 默认单次上限；已有显式预算优先，现有 $100 日预算与 17 份模型配置不变。尚未部署或宣称 Ask 修复通过。
