...
Current checkpoint (2026-09-12 23:02 UTC): R20 `b994b09c` is published and both owner pointers verify against its exact acceptance chain. The frozen source passed 7,831 tests (zero failures/errors, one skip), wheel 608/three-JS verification, copied-state 14 steps/42 ticks/zero escapes, and 45/45 healthy samples over 669.77 seconds. Runtime/configuration verification preserves all 17 model configurations and the installed R18b broker/host dependency. Actual product acceptance is separate: CTSH persisted one newly validated model specification under R20; no new model version existed at the 23:01:35 product snapshot. IBM's final repair still fails exact historical formula ties. Next-wave prompt guidance is integrated at `f8249f31` with 113 focused tests passed, but remains undeployed. Sol parallel work continues on strict filed-subtotal diagnosis, actual downstream products, and faster child-result settlement without increasing full planner runs. R19's failed suite and R18b's failed health window remain immutable. No model preference, uncertain reservation, 6000-token post-response ceiling or source/human authority changes. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
