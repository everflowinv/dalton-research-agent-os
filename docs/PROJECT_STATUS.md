...
Current checkpoint (2026-09-12 20:01 UTC): R17a remains the deployed verified runtime. R18 source `b0a5c35e` passed independent runtime/broker/ops review and wheel608/three-JS verification, but was superseded before acceptance or deployment when root identified an old single-plugin/index assumption and missing host-artifact relocation in the copied-state rehearsal. Its full suite was intentionally interrupted after 113.5 seconds; that interruption is not reported as an assertion failure. Integration `29be3d9e` corrects rehearsal confinement for declared plugin mutations and host artifacts; 154 related tests pass. R18a will freeze the corrected runtime and ops together and rerun the complete acceptance sequence. The release fixes downstream failure authority, terminal model-lane restarts/fairness, exact local no-send evidence, explicit verifier transport controls/fallback, and financial role/unit output guidance. IBM's producer policy remains maxTokens hint plus post-response ceiling; the verifier guard does not prevent that producer call. No budget, model preference, uncertain reservation or source/human authority is changed. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
