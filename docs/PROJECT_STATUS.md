...
Current checkpoint (2026-09-11 09:27 UTC): Live R9c and initial host cooldown remain active. Candidate annual execution now covers real production budget authority, long queues/safe retries, post-send unknown evidence and separately budgeted recovery; 104 integrated annual tests passed. Common document/Cockpit retries passed 144 tests, broker 32 tests, and custom planner retry/transport/effective-budget wiring passed 152 root integration tests. The three preflight2 annual schema failures are fixed; 34 affected portable/packaging/production tests passed. Actual setup on a copied Router proved the exact 12→14 delta: twelve provider additions, eleven transport additions, two new annual configs, all other existing fields unchanged. A new immutable full-suite/wheel/copied-state acceptance is next; nothing from this release scope is deployed yet. Persistent host isolation, AlphaEngine selection and the provider bridge retain separate activation gates. Other custom model consumers and long-lived planner configuration reload remain scheduled follow-ons; all-company deliverable completion is not claimed.
