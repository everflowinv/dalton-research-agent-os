...
Current checkpoint (2026-09-12 19:36 UTC): R17a remains deployed and runtime-verified (7,730 tests / one skip; 45/45 health samples). R18 now integrates truthful downstream dispatch failures, terminal model-lane holds/fairness, and exact Work/attempt/route/phase budget no-send proofs. Combined runtime/audit checks passed 169 tests; the independently reviewed legacy local-refusal compatibility adds a passing 16-test audit suite. The actual ChatGPT/Codex transport removes `max_output_tokens` before dispatch, so advertising a provider output hard limit was incorrect. Three GPT-5.6 Sol agents are implementing truthful per-transport required controls, independently reviewing failover and evidence, and extending the closed broker artifact deployment/rollback contract. The prior IBM no-send conclusion is superseded: broker evidence records 6,843 output tokens against 6,000 and broker-reported cost $0.80651. Its uncertain reservation remains held. R18 has not been frozen or deployed; broker artifact changes require their own exact deployment proof and cannot be hidden in a pure-preserve runtime receipt. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
