...
Current checkpoint (2026-09-11 21:55 UTC): R15 `d1079bafc565e1bfa7d19d3360504fa845e9b327` is frozen and pushed but failed full discovery (7,648 tests, 2 failures, 15 errors, one skip). All 17 came from missing filed unit/period fields in three fake fixtures; the corrected 49-test subset now passes without weakening source validation. Its exact 606-file wheel and 14-step/42-tick/zero-escape rehearsal passed, as did final 110-cell AMZN-style combined numerical/visual QA. Two additional product checks hold deployment: all five real legacy CLI exports have input-hash drift from the new series projection, and a new spec can become current before real historical formula replay. Sol agents are fixing both in parallel, with independent review next. The earlier 500-cell legacy check remains valid only for stored model reads/recomputation; it did not prove production input reconstruction. Live remains verified R14c1, with its daily-budget recovery repair still pending deployment. Next: finish both fixes, freeze a successor, pass exact-source full acceptance, deploy and observe health, then verify normal research recovery and new model production. Numeric-period/note context, complete statement/segment/category/Street breadth and human source signatures remain separate follow-ups. See [R15 candidate acceptance](reports/foundation-r15-candidate-acceptance-2026-09-11.md) and [AMZN format acceptance](reports/fund-amzn-template-parity-review-2026-09-11.md).
