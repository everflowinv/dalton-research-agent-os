...
Current checkpoint (2026-09-11 15:41 UTC): R12a `f9f93e3d` is installed with exact 598-file/17-config verification, but sustained health FAILED: 45 observations over 670.738 seconds, one stale heartbeat (31.280 seconds), same controller throughout. No successor release pointer is published; retain predecessor and fresh rollback. The installed planner also refuses before send because the expanded original-document inventory exceeds its old input bound; its normal child bypasses the Cockpit budget override. Sol is fixing that exact config-consumption seam and investigating the controller stall. Integration `aed177a5` includes independently reviewed model-history/sensitivity gate fixes, OpenClaw next-tick public catalog synchronization, and the root financial-model state projection (five companies actually hold 12 model history quarters; document-figure counts must not obscure them). Next: close these runtime/product defects, integrate tests and deploy an accepted successor; then re-audit all owner instructions and postponed work. **Latest owner direction supersedes the planned visual redesign: preserve the current Cockpit visual style and the subtask/run-detail design.** Improve plain-language copy and information hierarchy; hide Claim/Evidence internals behind optional details. Reference deliverables and multi-workspace work remain after foundations; no research signature is implied.
