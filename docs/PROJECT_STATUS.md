...
**R24 正在安装，尚未宣称部署或 Ask 验收成功。** 冻结 runtime `7025`、OPS `2790` 的完整测试为 **7,949 tests、0 failures/errors、1 skip**，另有 **134 项 OPS 检查**通过；608 个运行文件、17 份模型配置、14 步复制状态演练和部署前 preflight 均已通过。Ask 修复已合回通用集成 `c93ce1b6`，最终状态仍须等待本次安装、运行健康和真实 Ask 验收。

Owner 将当前范围收敛为“优先开发没有就跑不起来的内容，然后部署”。当前已发布基础版本仍为 **R23 `3e4e8682`**：完整 **7,945 tests、0 failures/errors、1 skip**，608 个运行文件及 17 份模型配置核验通过，14 步复制状态演练通过，45/45 持续健康采样通过并正式发布。R22b 安装失败与健康回滚、R22c 运行通过的历史证据保留。
...
**R23 的实际 Ask 测试曾被模型路由拒绝。** 原因是问答复用了 extraction 配置，而 Ask 属于 brain tier；配置缺少该链允许的模型和 credential slot，且通用默认单次 $0.05 无法覆盖约 104.6k 输入。R24 候选改为使用现有研究规划配置，并为交互实例设置 $2 默认单次上限；已有显式预算优先，现有 $100 日预算与 17 份模型配置不变。代码与候选验证通过不等于线上 Ask 已成功。

[Demo 使用说明](DEMO.md) 列出了交互研究台与公开只读监督台的不同入口。正式交付还需完成上述问答修复、同版验证与一次真实回答验收。
