...
Current checkpoint (2026-09-12 22:05 UTC): R18b frozen `3855c1e3` is installed, but its continuous health acceptance failed (43/45 healthy samples); owner publication remains R17a. R19 integration now contains fair outbox/planner writer scheduling, financial structure 0.4 for the filed IBM gross-profit-to-pretax bridge, and a version-bound correction that prevents cumulative subtraction of weighted-average shares. The integrated 262-test regression passed; independent replay review confirmed old outer-spec-0.4/inner-structure-0.3 input bytes are unchanged. New state/task identities prevent old rejected responses from being reused. Task-audit correction `09812641` now verifies explicit transport-policy metadata and suffix order; historical suffixes without persisted policy authority remain explicitly unknown. Its 32 direct regression tests passed. R19 still requires one clean source/ops freeze, full suite, wheel/copy rehearsal, installation and a fresh complete continuous-health window; no successor acceptance is claimed. Its runtime-only pure-preserve transition must verify the installed R18b broker/host dependency without reapplying those patches. Current runtime recovered, but the failed R18b health evidence remains immutable. No model preference, uncertain reservation, 6000-token post-response ceiling or source/human authority changes. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
