...
Current checkpoint (2026-09-11 11:13 UTC): R11a is frozen at `424c22529d5317c4c5c9dc8ba88f37299cea3f49`; guarded full-suite acceptance is running. Its wheel SHA is `418bf77f7b5285f9e53e041bdcf9d36463e57a7fa0bd4ce368ded2ec1e50d046`; 572 runtime files match source and three JavaScript checks pass. The freeze includes verified configurable backup retention, single-flight background backup, Dossier cited-date/gap handling, immutable host test fixtures and supported/manual Agenda post-send-unknown persistence (live Agenda remains retired). Root integration checks passed: async backup/service/writer/rehearsal 187 tests / 8.015 s; Agenda/adapter/fallback 73 / 2.328 s; Dossier 274 / 16.512 s. R11 failed acceptance remains recorded. Live remains R10a `7e3fb7b`: Alpha v2 stopped-copy validation found retained budget WAL/SHM before any target write, and automatically restored healthy services; postfailure health at 11:09:43 confirms Controller 10589 and exact 14 model configs. Alpha v3 now tests both cold and retained nonempty WAL using exact raw main/WAL/SHM copies, replaying only the copies; activation is still pending. User-authorized deletion already freed 18.45 GiB from 23 old runtime backups plus 18 old deployment archives. `backup.keep_latest=3` awaits R11a deployment. Parallel followups address Dossier gaps reaching the planner and bounded correction of slightly overlength company-model assessments. Higher investment capabilities, final visual/reference deliverables and multi-workspaces remain after foundations.
