...
本轮末次逐项核对新增修复：下载名使用公司、类型与日期；49 条审批全部显示可读业务详情，原有操作保持独立；研究 HTML 的内部状态、指标名、成本模板问题已中文化，引用编号移入折叠详情；页面各章节资料缺口共用准确的中文显示。只读真实候选验证中，49/49 审批详情可见，五家公司 HTML 指定内部词残留为零。仍在补查“阅读完整报告”的独立入口，防止打开未审校原文。
