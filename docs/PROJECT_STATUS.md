...
Current checkpoint (2026-09-11 16:17 UTC): R13a runtime `1afbeb76` is frozen with 599-file wheel verification and its full suite running, but WILL NOT be deployed: the newly approved numerical-consensus source exposed a real annual/quarter comparison bug. The existing bridge joins `(metric, period_end)` without period kind, so a Q4 model result can be compared to full-year Street revenue. This affects existing code, not the validity of the newly fetched vendor data. A bounded fix is in progress with calendar-aware period matching, complete annual aggregation, explicit unavailable results for incomplete years, and rule/fingerprint changes to recompute old algorithm outputs. Async controller maintenance and actual Cockpit planner-budget consumption are already independently reviewed/merged; final runtime freeze will include the comparison fix. Ops preserve-existing mode is separately under review for exact configuration bytes, real installer rehearsal, concurrent-edit handling and recoverable failure receipts. Live R12a remains installed/bytes-verified but failed sustained health; no new release pointer is published. The owner-approved Yahoo source is fetching successfully along the normal queue. After this pipeline closes, re-audit all earlier owner requirements and postponed work. Preserve the current Cockpit style and subtask/run-detail design; improve plain-language copy and information hierarchy, with technical details optional. The earlier full visual redesign is canceled.
