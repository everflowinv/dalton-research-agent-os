...
Current checkpoint (2026-09-11 16:47 UTC): R13b `6a2631ff489ac888b363cf26e24949171ce61fad` is frozen in a clean detached acceptance worktree. The annual/quarter consensus comparison correction passed independent review and 345 focused tests: exact fiscal quarters, filed revenue actuals plus formal forecast revenue, four-quarter annual coverage and compatible units. All five real-state previews now compare complete current-year revenue; EPS remains explicitly unavailable because there is no formal share-count/per-share model output. Old stored comparison projections are invalidated by the new rule binding, not rewritten in place. Root full-suite acceptance, independently verified wheel and real copied-state installer rehearsal run in parallel against this same source. R13a's 7,407-test run had one obsolete fixture failure and is preserved as failed; its fixed fixture is included in R13b. Live R12a remains installed but failed sustained health; no new accepted release pointer has been published. The owner-approved Yahoo source has successfully fetched all five companies through the normal queue. R13b preserves the exact 17 model configurations, mission v14 and approved source authority; the reviewed service change configures the actual planner call ceiling (250,000 input bytes / 4,000 output tokens / $3 / 300 seconds), with the daily mission budget unchanged. Next: finish same-source acceptance, deploy, verify installed state and sustained health, then re-audit all earlier owner instructions and postponed product work. Preserve the current Cockpit visual style and subtask/run details; improve plain language and information hierarchy with technical details optional. The earlier full visual redesign is canceled.
