...
Current checkpoint (2026-09-11 23:30 UTC): R15b `5e1d159bf99da02c8e456d07b4d54141b4f32c7c` passed source/Excel acceptance and installed correctly, but its first sustained health observation FAILED: the parallel R16 copied-state rehearsal exhausted the host disk, preventing heartbeat writes and causing a controller restart. Failed health/test/rehearsal evidence remains preserved; no verified publication occurred. Root removed the failed scratch and, after verifying all databases and restore copies in the latest three rollback snapshots, deleted two obsolete deployment backups (5,517,748,590 bytes). Four live database quick checks passed. The identical accepted R15b packet was reinstalled at 23:27–23:28 UTC; installed bytes/configurations match, and a new same-process health observation is running. R16 numeric-context code is independently reviewed and integrated, but its 7,670-test run failed during ENOSPC (3 failures/791 errors) and its copy rehearsal failed; neither is accepted. Next: review the new configurable rehearsal disk guard (124 tests pass), finish clean runtime recovery acceptance, restore sufficient headroom, then rerun successor validation. Three Sol agents continue typed financial-note target/evidence/period-specific EPS replay; full production model plumbing follows. No annual-only note may imply quarterly readiness. See [R15 recovery](reports/foundation-r15-candidate-acceptance-2026-09-11.md) and [numeric context](reports/company-model-source-bound-numeric-context-2026-09-11.md).
