...
Current checkpoint (2026-09-12 23:24 UTC): R20 `b994b09c` remains published with 7,831 tests passed (one skip) and 45/45 healthy samples over 669.77 seconds. CTSH, EPAM, ACN and DXC have now persisted four new model specifications under the exact R20 task; IBM remains refused by strict formula validation. Forecast models/lines remain zero: CTSH's forecast attempt failed local filing reconciliation for diluted EPS; the exact input and source-row selection are being diagnosed. Next-wave integration `ce3ab25f` adds filed-bridge prompt contract 0.14, auditable transport metadata for new model-spec Work with exact legacy replay, and 15-second settlement-only polling that preserves full planner/outbox priority and creates no new provider work. Root's combined 352 tests plus 35 writer/acceptance tests passed; independent reviews passed. These changes are pushed but not deployed or full-suite accepted. R21 release helpers remain inert/unbound while the forecast defect is investigated. Model preferences, source/human authority, uncertain reservations and the 6000-token post-response ceiling remain preserved. See [resumed development and next steps](reports/session-resume-r16a-financial-notes-2026-09-12.md).
