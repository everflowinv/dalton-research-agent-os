...
class AgendaCoordinator:
    ...
    def _record_usage_and_cost(
        ...
        if not token_split_available:
            if route.get("selected_profile_version_ref") != profile.get("profile_version_ref"):
            ...
            if len(selected) != 1 or not route_ref:
                raise CoordinatorError("route estimate is missing or ambiguous")
            try:
        ...
        usage_entry = client.record_usage(
            ...
            metering_source=(
                "provider_reported" if token_split_available else "launcher_measured"
            ),
        ...
        for metric, charge in (
            ...
            # Create token rate heads even when provider telemetry omits the
            # split. A successful invocation advances the model profile, and
            # the next token rate version must have an existing prior head.
        ...
        if token_split_available:
            ...
            rate = client.create_price_rate_version(
                ...
                charge_type="request", unit_quantity=1,
                unit_price_micros=amount_micros, currency="USD",
                effective_from=invocation.created_at, effective_until=None,
                source_ref=route_ref, actor_ref=COORDINATOR_ACTOR,
