...
def execute_alphaengine_search_probe(work_order: Mapping[str, Any], *, client: Any,
    ...
    succeeded = (record.get("status") == "succeeded"
                 ...
                 and isinstance(search.get("connector_invocation_ref"), str)
                 and isinstance(search.get("connector_invocation_hash"), str)
                 and isinstance(search.get("source_envelope_ref"), str)
                 and isinstance(search.get("source_envelope_hash"), str))
    return {
