...
from build_sec_8k_discovery_proposal import (_next_plan_id, build_review_bundle,
    read_active_mission, verify_mission_authority_on_backup)
from dalton_core.macos_launchagent import SEC_PLAN_SELECTOR
from dalton_core.mission_source_discovery import SEC_SOURCE_REF, validate_discovery_plan
...
def checked(path:Path, expected:str)->dict[str,Any]:
    raw=path.read_bytes()
    if hashlib.sha256(raw).hexdigest()!=expected: raise ValueError(f"sha256 mismatch: {path}")
...
def validate_packet(*,active_path:Path,active_hash:str,candidate_path:Path,candidate_sha:str,
                    active_sha:str,selector_path:Path,selector_sha:str,source_core:Path,
                    governance_path:Path,governance_sha:str)->dict[str,Any]:
    active=validate_discovery_plan(checked(active_path,active_sha))
    ...
    if (selector['schema_version']!='sec-discovery-plan-selection-0.1'
        or selector['source_ref']!=SEC_SOURCE_REF): raise ValueError('selector schema/source is invalid')
    relative=Path(selector['plan_path'])
    if relative.is_absolute() or len(relative.parts)!=1: raise ValueError('selector plan_path must be a plain filename')
    if candidate['id']!=_next_plan_id(active['id']): raise ValueError('candidate must be the next active plan version')
    ...
    governance=checked(governance_path,governance_sha)
    build_review_bundle(active_plan=active,candidate_plan=candidate,mission=mission,
                        governance=governance,created_at='1970-01-01T00:00:00+00:00')
    verify_mission_authority_on_backup(source_core,mission)
...
def run(args)->dict[str,Any]:
    packet=validate_packet(active_path=args.active_plan,active_hash=args.active_plan_hash,
      active_sha=args.active_plan_sha256,candidate_path=args.candidate,
      candidate_sha=args.candidate_sha256,selector_path=args.selector,
      selector_sha=args.selector_sha256,source_core=args.source_core,
      governance_path=args.governance,governance_sha=args.governance_sha256)
    ...
    receipt_path=args.target_dir/'sec-filings-plan-selection-v1.approval.json'
    if args.command=='apply' and args.actor and receipt_path.is_file():
      old=json.loads(receipt_path.read_text())
      approved_at=old.get('approved_at')
    else: approved_at=datetime.now(timezone.utc).isoformat(timespec='microseconds')
    receipt={'schema_version':'sec-discovery-selection-approval-0.1','actor_ref':args.actor,
      'approved_at':approved_at,'mission_binding':report['mission_binding'],
      'prior_plan':report['prior_plan'],'candidate':{'ref':packet['candidate']['id'],'hash':packet['candidate']['content_hash']},
      'selector':{'ref':packet['approved_selector']['id'],'hash':packet['approved_selector']['content_hash']},
      'governance_binding':{'ref':packet['governance']['id'],'hash':packet['governance']['content_hash']}}
    receipt['content_hash']=content_hash({k:v for k,v in receipt.items() if k!='content_hash'})
    receipt_bytes=(json.dumps(receipt,sort_keys=True,separators=(',',':'))+'\n').encode()
    report['approval_receipt']={'target':str(receipt_path),'target_state':state(receipt_path,receipt_bytes)}
    conflicts=[x for x in ('candidate','approval_receipt','selector') if report[x]['target_state']=='conflict']
    ...
    if args.command=='apply':
      ...
      report['approval_receipt']['write']=atomic_create(receipt_path,receipt_bytes)
...
def main(argv=None):
 p=argparse.ArgumentParser(); p.add_argument('command',choices=('prepare','apply')); p.add_argument('--active-plan',type=Path,required=True); p.add_argument('--active-plan-hash',required=True); p.add_argument('--active-plan-sha256',required=True); p.add_argument('--candidate',type=Path,required=True); p.add_argument('--candidate-sha256',required=True); p.add_argument('--selector',type=Path,required=True); p.add_argument('--selector-sha256',required=True); p.add_argument('--governance',type=Path,required=True); p.add_argument('--governance-sha256',required=True); p.add_argument('--source-core',type=Path,required=True); p.add_argument('--target-dir',type=Path,required=True); p.add_argument('--actor'); p.add_argument('--execute',action='store_true'); p.add_argument('--service-stopped-ack',action='store_true')
