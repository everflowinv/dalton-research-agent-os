...
def main():
 ...
 if not a.execute:
  data=(canonical_json(plan)+'\n').encode();fd=__import__('os').open(a.output,__import__('os').O_WRONLY|__import__('os').O_CREAT|__import__('os').O_EXCL,0o600);__import__('os').write(fd,data);__import__('os').fsync(fd);__import__('os').close(fd);return
 ...
 rw=sqlite3.connect(str(a.journal_db),isolation_level=None);out=recover_failed_ask_job(rw,job_id=a.job_id,expected_failed_hash=a.expected_failed_hash,producer_result=producer,result=result,recovery_proof=proof,artifact_dir=a.artifact_dir,receipt_path=a.receipt,recovered_at=a.recovered_at);rw.close();data=(canonical_json({**plan,**out,'status':'complete'})+'\n').encode();fd=__import__('os').open(a.output,__import__('os').O_WRONLY|__import__('os').O_CREAT|__import__('os').O_EXCL,0o600);__import__('os').write(fd,data);__import__('os').fsync(fd);__import__('os').close(fd)
